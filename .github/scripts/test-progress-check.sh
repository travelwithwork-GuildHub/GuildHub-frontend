...
# mkdone <標記那一格的內容>：追加一張表，項目 EVD-A01 帶著指定的標記。
# `Done` 是整張表裡**唯一一個人手寫的「事實」**，所以它要有自己的 fixture。
mkdone() {
  {
    printf '\n## EVD 證據\n\n'
    printf '| ID | 項目 | 工作 | 週 | 點 | 阻塞 | 標記 |\n'
    printf '|---|---|---|---|---|---|---|\n'
    printf '| EVD-A01 | 治理工作 | 做事 | W1 | 3 | | %s |\n' "$1"
  } >> "$W/docs/WBS.md"
}
...
# **fixture 從 AGENTS.md 搬到 ROADMAP.md。**
# 2026-09-08：引用掃描縮到只掃 docs/WBS.md 與 docs/ROADMAP.md 兩份結構化文件，
# 散文（AGENTS.md／CLAUDE.md／README.md／CONTEXT.md）完全不掃。
# 這些案例驗的是**掃描器的文法**（同形字、圍籬、註解、群組引用），
# 那件事仍然要驗 —— 只是要在還會被掃的檔案裡驗。
edit_agents()  { edit_in docs/ROADMAP.md "$1" "$2"; }
...
# 現在有兩層：`Pd` 類折成 `-`（於是 `FE﹣Q99` 直接被當成 `FE-Q99` 驗），
# 非 `Pd` 的相似字元**不折**，由〈認形狀不認字元〉那條報出來。
# **兩種結局都不是靜默** —— 差別只在「當成正確」與「叫你改成 ASCII」。
baseline
edit_agents "\`FE-C01\` 是工作項目" "\`FE-C01\` 與 \`FE─Q99\` 是工作項目"
run 1 "U+2500 製表線寫的 ID 要報形狀" "不是連字號"

baseline
edit_agents "\`FE-C01\` 是工作項目" "\`FE-C01\` 與 \`FEーQ99\` 是工作項目"
run 1 "U+30FC 日文長音寫的 ID 要報形狀" "不是連字號"
...
#
# 這一格**機器沒有驗過任何東西**（不像其他狀態是從 git 與 OpenSpec 推的），
# 所以能鎖住的只有三件事：它真的生效、它要有理由、它不准跟 change 並存。

baseline
mkdone "Done｜governance PR #58，commit ae78a12c"
run 0 "標 Done 的治理項目：綠" ""
run_field_has "而且狀態真的是「已完成」" "EVD-A01" "state" "已完成"

# **叫「已完成」不叫「已封存」。** 借用 OpenSpec 的字會讓兩種強度不同的
# 結論長得一樣 —— 一個是 git 證明的，一個是人宣告的。上面那條
# `run_field_has "已完成"` 就鎖住了它：改成「已封存」它會紅。

# 沒有理由的 Done，六個月後沒有人知道憑什麼。
baseline
mkdone "Done"
run 1 "Done 沒有寫理由要紅" "理由"

# **有 change 就不准標 Done。** 狀態算得出來，兩個來源就會漂 ——
# 把矛盾攤出來，不要挑一邊信。
baseline
mkchange evd-a01-x
mkdone "Done｜其實有 change"
run_field_has "有 change 還標 Done：報矛盾" "EVD-A01" "state" "矛盾"

# 互斥性：Done 跟其他處置不得並存（沿用既有的互斥檢查）。
baseline
...
# 這一條改的是**來源**（多開一個 change），區塊故意不重產。
#
# 2026-09-08 降級：區塊過期**不再是違規**（`--check` 仍然 rc=0），
# 因為那個要求逼每個 spec／archive PR 都得順手重產一次區塊 ——
# 而那兩種分支原本不准碰 docs/WBS.md，於是又逼出一條路徑例外。
# **一條檢查逼出一條例外，就是規則互相牽制的開始。**
# 現在驗的是「它會說」，不是「它會擋」。
baseline
mkchange fe-c01-x
mkmarkers
render
mkchange fe-p03-y
run 0 "來源變了但區塊沒重產：不擋門，但要說出來" "進度區塊跟現在的狀態對不上"

# 重產之後連提醒都不該有 —— 沒有這條的話，「永遠提醒」也會讓上一條通過。
render
run_absent 0 "重產之後不再提醒" "進度區塊跟現在的狀態對不上"
...
# **標 Done 的項目在區塊裡要說出狀態是哪裡來的。** 它沒有 change 可以指，
# 那一格如果是「—」，看起來像資料掉了。
baseline
mkdone "Done｜governance PR #58"
mkmarkers
render
run_block have "Done 的項目在區塊裡指得出來源" "標記"
...
run 1 "U+2212 減號寫的 ID 要報形狀" "不是連字號"
...
run 1 "ROADMAP 提到不存在的群組：紅" "群組 FE-D 不存在"
...
# ── 正本表頭放錯地方（GuildHub #671 實測）────────────────────────────
# 把舊表**原地**改成四欄、沒加 `## 跨項依賴` 標題：正本只認標題底下的表，相容層只認
# `| 這一項 | 依賴 |` 兩欄表頭，兩邊都不認 —— rc=0、連「沒有被驗」都不講。
# 「原地遷移」是最自然的做法，所以要紅。
baseline
edit "| 這一項 | 依賴 |
|---|---|
| \`FE-P03\` 清單 | \`FE-C01\` 骨架 |" "已知的跨項依賴（改動時要一起看）：

| 這一項 | 依賴 | 關係 | 說明 |
|---|---|---|---|
| FE-P03 | FE-C01 | 全部 | 清單要先有骨架 |"
run 1 "原地把舊表改成四欄（沒有 ## 跨項依賴 標題）要紅" "不在「## 跨項依賴」這個標題底下"
run_expr "…原因碼是 cross_dep_table_not_parsed（網頁提示跑 --check），不是 no_cross_dep_table" \
  "x['reason']=='cross_dep_table_not_parsed' and x['schema'] is None"
...
## 里程碑"
run 1 "正本表放在 ### 跨項依賴 底下要紅（要 ## ）" "不在「## 跨項依賴」這個標題底下"

# 對照：放對地方的正本表不報這條。
baseline
xdep "| FE-P03 | FE-C01 | 全部 | 清單要先有骨架 |"
run_absent 0 "對照：正本表在 ## 跨項依賴 底下，不報放錯地方" "不在「## 跨項依賴」這個標題底下"

# 對照：〈跨項依賴〉底下表頭欄位錯的，只報「認不出來」那一條，不重複報放錯地方。
baseline
edit "| \`FE-P03\` 清單 | \`FE-C01\` 骨架 |" "| \`FE-P03\` 清單 | \`FE-C01\` 骨架 |
...
|---|---|---|
| FE-P03 | FE-C01 | 全部 |"
run_absent 1 "表頭欄位錯（在標題底下）：不重複報放錯地方" "不在「## 跨項依賴」這個標題底下"
