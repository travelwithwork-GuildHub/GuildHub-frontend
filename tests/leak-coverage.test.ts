...
describe('洩漏偵測的涵蓋率', () => {
  it('src/world/ 底下每一個會建立 GPU 資源的模組都在受測清單裡', () => {
    ...
    const listed = listedSources(fs.readFileSync(HARNESS, 'utf8'))
    ...
    expect(coverageProblems({ scanned, listed })).toEqual([])
  })
  ...
  it('[FE-W07-S06] 新增會建立 GPU 資源的元件而沒有登記時，變紅', () => {
    ...
    const problems = coverageProblems({ scanned, listed })
  ...
  it('[FE-W07-S07] 清單裡列了不存在的檔案時，也要紅', () => {
    const scanned = scanWorld()
    const listed = [...listedSources(fs.readFileSync(HARNESS, 'utf8')), 'src/world/Gone.tsx']
