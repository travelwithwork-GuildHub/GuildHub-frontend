...
// 所以每一條都：把 `src/api/contract/` **原封不動複製一份**、在複本上改、
// 對複本跑真的 tsc。
//
// ⚠️ **不能改原地的檔案。** 第一版是那樣寫的，結果 `typecheck-negative.test.ts`
// 的陽性對照紅了 —— 它跑的是整個專案的 `npm run typecheck`，而 vitest
// 的測試檔是並行的，兩邊撞在一起。那支測試的註解早就寫過這件事
//（「兩個並行的 vitest run 會互相覆寫與刪除」）。
//
...
// `tests/drift-scope/__tmp_*` 在主 tsconfig 的 exclude 裡 ——
...
/**
 * 把契約複製一份、套用改寫、對複本跑 tsc。
 *
 * `edit` 收到檔名與內容，回傳改寫後的內容。回傳原文代表不改那個檔案。
 */
function typecheckCopy(edit: (file: string, src: string) => string) {
...
describe('後端漂移哨兵', () => {
  it('[FE-O01-S10] 契約與產出的型別一致時是綠的（陽性對照）', () => {
    const { code, output, changed } = typecheckCopy((_f, src) => src)
    expect(changed, '這一條不該改動任何東西').toBe(false)
    // **沒有這一條，下面每一條都是恆真的** —— 一條永遠 false 的斷言
    // 會讓所有負向測試通過，而且它會讓 CI 永遠紅到被人註解掉。
    expect(code, `契約本來就是紅的：\n${output}`).toBe(0)
  }, 60_000)

  it('[FE-O01-S10] 登錄表少一個實體就會紅，而且紅在涵蓋率那一條', () => {
    const { code, output, drift, changed } = typecheckCopy((file, src) =>
      file === 'drift.ts' ? src.replace('  RoomDoorOut: rest.RoomDoorOut,\n', '') : src,
    )
    // 取代沒發生的話，這條測試會驗到一份沒改過的複本然後恆真。
    expect(changed, '對 drift.ts 的改寫沒有生效 —— 登錄表的寫法可能變了').toBe(true)
    expect(code, '從登錄表移掉一個實體之後 typecheck 竟然還是綠的').not.toBe(0)
    // 只看退出碼不夠：**紅的原因可能跟涵蓋率一點關係也沒有**。
    const coverage = lineOf(drift, '_coverage')
    expect(redAt(output, coverage), `紅了，但不在涵蓋率那一行（${coverage}）：\n${output}`).toBe(true)
  }, 60_000)

  it('[FE-O01-S11] 後端把必填欄位改成可為 null 時會紅，而且指向那個實體', () => {
    const { code, output, drift, changed } = typecheckCopy((file, src) =>
      file === 'schema.d.ts'
        ? src.replace('            online_count: number;', '            online_count: number | null;')
        : src,
    )
    expect(changed, '對 schema.d.ts 的改寫沒有生效 —— 產出的格式可能變了').toBe(true)
    expect(code, '產出的型別欄位變成可為 null 之後 typecheck 竟然還是綠的').not.toBe(0)
    const roomDoor = lineOf(drift, '_RoomDoorOut')
    expect(redAt(output, roomDoor), `紅了，但不在 RoomDoorOut 那一行（${roomDoor}）：\n${output}`).toBe(true)
  }, 60_000)

  it('[FE-O01-S11] 後端新增一個欄位時也會紅 —— 新欄位是契約變更，不該被吞掉', () => {
    const { code, output, drift, changed } = typecheckCopy((file, src) =>
      file === 'schema.d.ts'
        ? src.replace(
            '            online_count: number;',
    ...
    expect(changed, '對 schema.d.ts 的改寫沒有生效 —— 產出的格式可能變了').toBe(true)
    // 單向的 `extends` 會讓這一條靜靜通過。用雙向 `Equal` 就是為了這個。
    expect(code, '產出的型別多一個欄位之後 typecheck 竟然還是綠的').not.toBe(0)
    const roomDoor = lineOf(drift, '_RoomDoorOut')
    expect(redAt(output, roomDoor), `紅了，但不在 RoomDoorOut 那一行（${roomDoor}）：\n${output}`).toBe(true)
  }, 60_000)
})
