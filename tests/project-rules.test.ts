...
describe('上限由前端守，數字有出處，時機照全站規則', () => {
  ...
  it('[FE-J01-S03] 標題／內容超上限即時紅，數字是 FORM_LIMITS 的；單位是 code point', () => {
    // 60 個「𠮷」是 120 個 UTF-16 code unit —— 用 `.length` 數的實作這裡會紅
    ...
    expect(parse({ body: '字'.repeat(FORM_LIMITS.projectBody.max) }).success).toBe(true)
    expect(immediateIssues({ body: '字'.repeat(FORM_LIMITS.projectBody.max + 1) }, 'body')).toContain(String(FORM_LIMITS.projectBody.max))
  })
  ...
  it('[FE-J01-S03] 技能第 11 項、某一項 41 字即時紅；去空去重不分大小寫後算', () => {
    ...
    expect(immediateIssues({ skills: 'a, ' + 'x'.repeat(FORM_LIMITS.skillLength.max + 1) }, 'skills')).toContain(String(FORM_LIMITS.skillLength.max))
    expect(parse({ skills: 'x'.repeat(FORM_LIMITS.skillLength.max) }).success).toBe(true)
  })

  it('[FE-J01-S03] 座位數 0／7／2.5／空白即時紅（7 是 6+1）；範圍訊息裡的數字是 6 不是 8', () => {
