...
describe('進不去就回 Guild Hall、說一句話、不重試、票留著', () => {
  it('[FE-V01-S06] 10 秒沒有 hello 就回大廳（從 connect() 起算，等舊 close 那一段不算）；票還在、網址不多一層', async () => {
    ...
    expect(w.notice()).toEqual({ kind: 'failed', room: ROOM }) // （`enterRoom(ROOM)` 沒帶 title → 通知也沒有）
  ...
  it('[FE-V01-S07] 握手被拒：回大廳、不再試；通知留到使用者要求的成功進入／關閉／取代', async () => {
    ...
    await flush()
    expect(w.last().scene).toBe('lobby')
    expect(w.notice()).toEqual({ kind: 'failed', room: ROOM, title: '星際導航' })
    await tick(60_000)
  ...
  it('[FE-V01-S16] 回大廳也連不上時，不會永久 busy、沒有第三條連線', async () => {
    ...
    expect(w.probe().scene).toEqual({ id: 'hall' })
    expect(w.notice(), '回大廳失敗不再多一則').toEqual({ kind: 'failed', room: ROOM, title: '星際導航' })
    await tick(60_000)
  ...
  it('[FE-V01-S19] 上一頁進到進不去的房：失敗處置一樣，網址不留一層', async () => {
    ...
    await flush()
    expect(w.notice()).toEqual({ kind: 'failed', room: ROOM }) // （上一頁那條路沒有 title → 通知也沒有；`FE-N08-S11` 靠清單補房名）
    expect(url()).toBe('/world')
    await go(-1)
