...
try {
  ...
  if (free0 > 0) ok(`還沒開面板時按方向鍵，畫面差了 ${free0} 個像素（人在走）`)
  else bad('還沒開面板就走不動', '**這是量測壞了**：世界沒畫出來、或方向鍵沒送到。先修這裡')
  ...
  if (locked === 0) ok('[S18] 面板開著時按方向鍵，畫面 0 個像素的穩定差異（人沒有走）')
  else bad('[S18] 面板開著人還在走', `差了 ${locked} 個像素（對照組是 ${free0}）—— 鎖沒有到角色那裡`)
  ...
  if (free1 > 0) ok(`[S17] 關掉面板之後按方向鍵，畫面差了 ${free1} 個像素（人走得動）`)
  else bad('[S17] 關掉面板之後人走不動', '鎖沒有還回去 —— 使用者要用滑鼠點一下畫面。**只驗 S18 的話這一條是綠的**')
