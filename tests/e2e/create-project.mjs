...
//
// **不攔任何 `/api/*`。** `POST /api/projects` 只用 `page.route` 旁觀（`route.continue()`），
// 記下瀏覽器送出的 body 是哪幾個鍵 —— 攔下來偽造的話，「恰好四鍵」證明的是測試自己。
...
/** 每次跑都是新標題：同一個資料庫重跑時，上一輪的案子也在列表裡，「第一筆」要分得出是這一輪的。 */
const TITLE = `瀏覽器發的案 ${Date.now()}`
...
async function openProjectBoard(page, who) {
  ...
  // 列表載入中先等：`aria-busy` 放下來才讀第一筆，不然讀到的是上一頁或空的
  await page.waitForFunction(() => document.querySelector('[data-testid="list-panel-list"]')?.getAttribute('aria-busy') !== 'true', null, { timeout: 15_000 }).catch(() => {})
  return panel
}
...
const hasPostButton = (page) => postButton(page).isVisible().catch(() => false)
...
try {
  ...
  guardLoopback(first)
  const posts = []
  // 旁觀、不攔截：記下瀏覽器真的送出去的 body，然後放行到真的 Route Handler
  ...
  check('[S08] 列表第一筆是剛發的標題', await firstItemText(page), TITLE)
  ...
  await page.waitForFunction(() => document.querySelector('[data-testid="list-panel-list"]')?.getAttribute('aria-busy') === 'false', null, { timeout: 15_000 }).catch(() => {})
  check('[S08] 重新整理後從網址回來的面板，第一筆仍是那個標題', await firstItemText(page), TITLE)
  ...
  check('[S08] 重新整理、再開一次面板，第一筆仍是那個標題', await firstItemText(page), TITLE)
  ...
  check('[S08] 第二個人的面板第一筆是同一個標題', await firstItemText(page2), TITLE)
