...
**先判斷角色。** 提示第一行是哨兵 `【llm-team 寫手票】`（`.agents/skills/llm-team/write.mjs` 固定輸出的第一行）⇒ 你是**寫手**：只照票文與提示做、只准跑提示列出的指令頭、越界就停下回報，**不必**讀本檔其餘段落與 `AGENTS.md`。第一行是 `【llm-team 複審票】`／`【llm-team 規劃】` ⇒ 你是**複審者／裁決者**（`council.mjs` 以 `--mode plan` 唯讀叫起）：只答 Q 題、不執行指令、不讀檔。否則你是**統整者**，往下讀。
...
3. 派工一律用 `.agents/skills/llm-team/ticket.mjs`（流程入口 `.agents/skills/llm-team/SKILL.md`：`setup --check --coordinator agy` → `ticket run --coordinator agy` → 親自坐實複審 Q6 → `publish` 開 Draft PR），**不用 agy 內建 subagent 寫功能碼**；寫手是便宜模型、複審名單在 `llm-team.config.json` `profiles.agy`（只在 Claude 額度用完時才開這個模式：複審＝codex sol 一位、裁決交人；Gemini 桶 limit ⇒ 收工，不換模型硬撐），你自己不寫功能碼。
4. 開工先跑 `node .agents/skills/llm-team/setup.mjs --check --coordinator agy`，有紅就停。
