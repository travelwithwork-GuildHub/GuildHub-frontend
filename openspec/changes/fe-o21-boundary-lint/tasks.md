...
- [ ] 2.4 `FE-O21-S06`：`src/realtime/index.ts` 值再匯出 `./client` 報、`protocol.ts` 值 import／`import()`／`require()` `./client` 各報、`export type` 不報
- [ ] 2.5 `FE-O21-S04`：Scenario 列的 10 種寫法逐一跑；零 import 不報
- [ ] 2.6 `FE-O21-S05`：Scenario 列的 10 種寫法逐一跑；`limits.ts` 與 `contract/sub/x.ts` 報；`transport.ts` 不報
