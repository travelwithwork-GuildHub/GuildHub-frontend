...
- [ ] 2.1 `<button>`：名字、`avatarLook` 色塊、技能、時數；`null` 不是 0；沒有 `bio`／`updated_at`
- [ ] 2.2 判準：`S01`、`S02`、`S03`
- [ ] 2.3 **突變**：`hours_per_week ?? 0` → `S02` 要紅；把 `bio` 印上卡片 → `S03` 要紅
...
- [ ] 4.1 `BoardPanel` 人才那一支：`renderItem` 換成 `TalentCard`，選中的 id → `overlay` 放 `TalentDetail`
- [ ] 4.2 判準：`S04`、`S05`（Enter 與 Space）、`S11`、`S12`（含 not display:none）
- [ ] 4.3 **突變**：卡片改成 `div` → `S05` 紅；返回時卸載列表 → `S11`／`S12` 紅

## 5. 鍵盤與世界

對應 Requirement〈詳情層的鍵盤不驅動世界，Escape 之後詳情不再顯示〉

- [ ] 5.1 判準：`S13`、`S14`（真的 `LocalPlayer`）
- [ ] 5.2 **不改** Escape 的語意（`FE-X06` 的事）

## 6. 收尾

- [ ] 6.1 `npm run typecheck`、`npm run lint`、`npm test` 全綠
- [ ] 6.2 真瀏覽器：`tests/e2e/board-panel.mjs` 人才那一段多點一張卡、看詳情、返回；截圖進 `docs/evidence/fe-b04/`
