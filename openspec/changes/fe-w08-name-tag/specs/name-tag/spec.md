...
權限：不適用 —— 名字是協定廣播給同場景所有人的公開資料，本 change 不做授權判斷
併發：適用 —— render loop 每幀寫牌子的位置，同時 `presence`／`snapshot` 在改名單、`leave` 同步清掉樣本（`remote-players` 的兩個容器）
持久資料相容性：不適用 —— 不讀寫任何持久資料
失敗路徑：適用 —— `name` 空白或不是字串、名字超過牌子寬度、投影落在畫面外、名單上有人但樣本還沒到／已被清掉
測試連到什麼：jsdom 判準不連任何外部服務；真瀏覽器判準連**本機自起**的 `next start`，`/api/*` 用 `page.route` 偽造、
`/ws` 用 `routeWebSocket` 偽造（`tests/e2e/lib/world.mjs` 的 `fakeRealtime`，`others()` 給名單）。不連任何團隊共用的位址。

## 名詞

- **遠端玩家**：`remote-players` 名單（`RemoteIdentity`）上的人 —— 已排除自己（`FE-R05`）。
- **名字牌**：一個 DOM 元素，`data-testid="name-tag"`、`data-player="<id>"`，文字是那個人的 `name`。
- **頭頂錨點**：那個人的世界座標 `(x, y_head, z)`；`x`／`z` 是 `remote-interpolation` 求值出來的畫面位置（跟角色本身同一幀、同一份 `evaluate`），
  `y_head` 是角色頭頂再往上一段固定距離（design D3 給數字）。
- **投影**：世界座標 → CSS 像素，**跟門標籤同一份**（`labelProjection.ts` 的 `toScreen`）；不得另寫一份公式。
- **牌子尺寸**：固定寬 × 固定高（design D4 給數字），**不隨名字長度變**。
...
- **WHEN** snapshot 裡有一個人的 `name` 是 `"   "`、另一個是 `""`
- **THEN** 這兩個人都沒有名字牌
- **AND** 畫面上沒有任何一塊牌子寫著替代字
- **AND** 其他有名字的人的牌子照常
- **AND** 名單層直接拿到 `name` 不是字串（例如 `null`、數字）的人時，那個人也沒有名字牌（單元判準）
