...
- [ ] 4.1 輸入用該操作的契約 schema 驗，不合就拋錯
- [ ] 4.2 `FE-O02-S04`：不合契約時**不得送出請求**（測試 server 的請求數是 0）。
      **負向**：拿掉輸入驗證 → 請求會送出去，而且測試 server 的 Zod 會回 400

## 5. 回來的東西在邊界驗（`FE-O02-S05`～`S07`）

- [ ] 5.1 `FE-O02-S05`：回應少必填欄位 → 拋契約漂移錯誤，訊息含操作名與欄位。
      驗證：測試 server 故意回一個少欄位的 body
- [ ] 5.2 `FE-O02-S06`：回應多未知欄位 → 正常通過，而且回傳值裡沒有那個欄位。
      **這一條釘住 Zod 的 strip 行為** —— 有人加 `.strict()` 就會紅
- [ ] 5.3 `FE-O02-S07`：4xx／5xx 拋錯、帶 status、保留錯誤 envelope 的內容。
      **負向**：拿掉 `response.ok` 的檢查 → 這條必須紅
- [ ] 5.4 **負向**：拿掉回應的 `parse` → `S05` 必須紅
...
- [ ] 8.1 Profile：`GET /api/profiles/me`、`PATCH /api/profiles/me`
- [ ] 8.2 Project、Room、Seat、Message —— 以 `rest.ts` 已經涵蓋的端點為準，
      **不多做一個**
