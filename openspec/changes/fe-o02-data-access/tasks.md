...
- [x] 7.1 檢查：`src/api/` 出現那四個 domain 的操作、而契約層沒有對應 schema → 紅
- [x] 7.2 `FE-O02-S10`：契約層補上之後要放行。兩個方向各做一次。
      **另外抓到一個恆真的形狀**：原本用 `it.each(DOMAINS_WITHOUT_BACKEND)` 展開，
      把清單縮成 `['role']` 之後測試從 24 條變 21 條然後全過 ——
      那組測試是用「要被檢查的那份清單」當參數的。改成寫死一份對照
...
- [x] 8.2 Project、Room、Seat、Message —— 以 `rest.ts` 已經涵蓋的端點為準，
      **不多做一個**
- [x] 8.3 每個操作都有一條走真實路徑（測試 server）的測試，
      外加一條「清單涵蓋 `operations.ts` 匯出的每一個操作」—— 少了它，
      新增操作而忘記加進清單時什麼都不會發生
