...
- [x] 5.1 `FE-O14-S07`：`none` 時不建立 client。
      驗證：注入 socket 工廠替身，斷言**呼叫次數**（`none` → 0、`guildhub` → 1）
- [x] 5.2 `FE-O14-S07` 後半：`none` 時沒有任何錯誤或警告層級的診斷輸出
- [x] 5.3 `FE-O14-S08`：`none` + `production` + 無位址 → 掛載不拋錯，
      且世界的容器元素存在。**不要斷言「正常呈現」**（那是同義反覆）

## 6. 單人預覽的說明（DOM）

- [x] 6.1 `FE-O14-S09`：說明是 DOM、**可以用可存取名稱取得**、
      內容提到單人預覽、範圍內沒有任何按鈕或可點擊元素
- [x] 6.2 `FE-O14-S10`：`guildhub` 時不出現 —— 驗證方式 V5，
