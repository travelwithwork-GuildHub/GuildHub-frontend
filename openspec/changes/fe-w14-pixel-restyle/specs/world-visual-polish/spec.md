...
權限：不適用 —— 本 change 不做任何授權判斷，只動視覺呈現。
併發：適用 —— 像素貼圖依 color token 程序生成一次後快取，render loop 每幀讀共用材質；貼圖快取的建立 SHALL 是冪等的，不得每幀新建 GPU 資源。
持久資料相容性：不適用 —— 不讀寫任何持久資料。
失敗路徑：適用 —— 伺服器端／測試環境沒有 `document`（取不到 canvas 2D context）時貼圖 SHALL 退化為無貼圖的純色材質而不拋錯；`avatar_id` 越界／`null` 時角色外觀 SHALL 照常渲染（值域交給 `avatar-appearance`）。
測試連到什麼：不適用 —— jsdom 判準只斷言 React 樹與材質／幾何屬性，不連任何外部服務；真瀏覽器的可讀性判準連**本機自起**的 `next start`（`/api/*` 用 `page.route` 偽造），不連任何團隊共用的位址。驗證以**前後截圖**為主（web-facing 的可觀察成果，見 tasks）。

## 名詞

- **像素貼圖**：程序生成的 `CanvasTexture`（`NearestFilter`、`RepeatWrapping`），內容依一個 color token 的 hex 推出同色系的幾階明暗；**不載入任何外部圖檔**。
- **木紋貼圖**：`wood`／`woodDark` 專用的像素貼圖 —— 橫向順紋＋幾條深紋路＋一條板縫（跟一般雜訊貼圖不同）。
- **像素草地**：疊在主地面上的一張像素貼圖 plane，比材質雜訊更像草（多階綠＋草葉痕）。
- **像素化渲染**：以固定的**低有效 DPR**（`0.25`）、關閉 antialias 渲染，再靠 canvas 的 `image-rendering: pixelated` 最近鄰放大 —— 低解析度的畫面被硬邊放大成點陣外觀。這也是像素外觀的來源，同時把 GPU 要畫的像素數降到約 1/16（服務 `FE-X09`「弱裝置」）。
- **描邊**：inverted-hull —— 在一個部位後面疊一個放大一點、只畫背面（`BackSide`）的深色 box，露在本體外圈成一圈輪廓；不用後處理 pass。
- **框臉髮型**：頂＋瀏海＋兩側鬢角＋後腦組出的髮型（不是一塊平板）—— 這是「這是人」的關鍵訊號。
- **坐姿姿勢**：`ChibiPlayer` 的 `seated` 讓大腿往前彎、手往前擱、整體略抬到椅面高度。**只有姿勢**：坐在正確座位上的定位／面向／走動起身歸 `FE-J13`。
