...
**不看原本的 `status`**（真後端亦然：`active` 再成軍是換密碼、`closed` 再成軍會復活 —— 後者是 `FE-O08` 的 anomaly，由前端 UI 圍堵、契約套件不釘它）。
...
#### Scenario: [FE-J04-S12] form-team：owner 200 變 active 且能 enter；非 owner 403；未登入 401；不存在 404
...
#### Scenario: [FE-J04-S13] close：owner 200 變 closed、座位清掉、rooms 不含、重複 200；非 owner 403
