...
- [ ] 2.1 `tests/login-entry.test.tsx`：`S13` 暱稱路建立身分後有金鑰、「進入世界」disabled、**按之前** `localStorage` 沒有 `guildhub.first-entry-done`；
      剪貼簿替身（`clipboard` prop）回報成功後 enabled 且有「已經複製」；按下去 `router.replace('/world')` 且 `localStorage` 有 `guildhub.first-entry-done=1`；
      填回尾碼 6 碼填對也 enabled
- [ ] 2.2 同檔：`S14` 金鑰路成功後 `router.replace('/world')`（`push` 沒被叫）、DOM 上沒有 `recovery-key`、沒有金鑰文字
- [ ] 2.3 同檔：`S15` 剪貼簿替身 reject → 沒有「已經複製」、按鈕仍 disabled、`replace` 沒被叫、有手動保存的說明與填回入口；
...
- [ ] 3.3 `src/app/login/LoginForm.tsx`：刪 `RecoveryKeyPanel`；開 `clipboard?: ClipboardPort` prop 往下傳；暱稱路 `signed-in` → `KeyHandoff`
...
- [ ] 3.5 突變（先 commit）：`KeyHandoff` 的 `disabled={!done}` 拿掉 → `S13` 紅；渲染時就 `markFirstEntryDone()` → `S13` 紅；
      金鑰路改回顯示金鑰畫面 → `S14` 紅；金鑰路改 `push` → `S14` 紅；`copy()` 的 `setTaken` 搬到 `await` 前 → `S15` 紅；
      尾碼比對改「有填就放行」→ `S17` 紅；紀錄貼 PR
