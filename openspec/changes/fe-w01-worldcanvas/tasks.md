...
- [ ] 3.4 移除 `src/app/world/WorldPlaceholder.tsx`，**但不動 `WorldBoundary.tsx` 的殼**；驗證：`git diff` 顯示 `WorldBoundary.tsx` 未變更 —— 那道縫就是為了這件事留的
...
- [ ] 5.1 確認卸載時沒有殘留的全域事件監聽；驗證：測試包裝 `window.addEventListener`／`removeEventListener` 計數，斷言卸載前後相同（Scenario `FE-W01-S08`）
...
- [ ] 7.3 走完 `design.md`〈驗證方式〉的 **V1–V4**（production build），證據貼在實作 PR 上；驗證：四列都有輸出或截圖
