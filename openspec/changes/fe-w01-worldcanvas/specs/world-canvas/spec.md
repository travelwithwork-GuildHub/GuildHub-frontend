...
併發：不適用 —— 本 change 不引入共享可變狀態；render loop 尚未與網路資料互動
持久資料相容性：不適用 —— 不讀寫任何持久資料
失敗路徑：適用 —— WebGL2 不可用、3D 內容尚未載入完成
測試環境：**不適用 —— 測試不連任何外部服務。** 單元與 component 測試全部在
本機行程內執行，不開啟網路連線。人工驗證只連本機自己起的 production build

**⚠️ 這份規格有一部分在 jsdom 裡驗不到。** jsdom 沒有 WebGL，
所以「Canvas 真的在渲染」「陰影真的有出現」「畫布尺寸真的跟著變」
無法用 component test 證明。哪一條靠什麼證明，逐條寫在
`design.md` 的〈驗證方式〉，**沒有一條是留白的**。
