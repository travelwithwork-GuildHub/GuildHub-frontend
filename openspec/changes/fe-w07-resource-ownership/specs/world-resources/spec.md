...
併發：不適用 —— 釋放發生在 React 的卸載路徑上，沒有第二個寫入者
持久資料相容性：不適用 —— 不讀寫持久資料
失敗路徑：**適用** —— 元件自己建的資源沒有釋放（唯一量到的無聲失敗）、
量測台自己量不到東西（假綠）、瀏覽器或 server 起不來

測試會連到什麼：**不連任何外部服務。** 洩漏偵測載具是測試自己在
`127.0.0.1` 起的一個 vite server，當次建立當次銷毀，只服務 `tests/` 底下的檔案；
它不連後端，也不建立任何 WebSocket。這一點有 Scenario 擋（`FE-W07-S05`）。
