...
- [ ] 2.3 jsdom `tests/identity-random-avatar.test.ts`：`S21`（注入亂數 0／0.999／0.5 → 0／7／4；款數 3 → 0／2／1；沒注入時 spy `Math.random` 回 0.5 → 呼叫一次、送 4）、`S19`（金鑰／密碼登入不呼叫 `updateMyProfile`）、`S20`（`PATCH` 失敗回原 profile、只送一次）（Requirement〈首次建立身分時隨機指派一款外觀〉）
...
- [ ] 4.1 突變：`AVATAR_COUNT` 改回 2 → `S14`／`S22` 紅；某兩款同色 → `S14`＋`S16` 紅；亂數來源不呼叫（固定 0）→ `S21` 的「恰好呼叫一次」紅；預設來源改成 `() => 0`（保留注入路徑）→ `S21` 的 spy 那一半紅；金鑰路徑也 `PATCH` → `S19` 紅；失敗時重試 → `S20` 紅；`Math.floor(r × 8)` 寫死 8 → `S21` 的款數 3 那一半紅；拿掉 `flex-wrap` → `S23` 紅；`PATCH` 不 await 就導向 → `S17` 的順序紅
