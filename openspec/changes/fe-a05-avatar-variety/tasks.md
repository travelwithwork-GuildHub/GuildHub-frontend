...
- [ ] 2.4 真瀏覽器 `tests/e2e/first-entry.mjs`（或新 `avatar-assign.mjs`）：`S17`（暱稱建立 → 恰好一次 `PATCH`、body 只含 `avatar_id` 0～7、`PATCH` 回應先於 `/world` 導覽、進世界後自己的外觀是那一款；十二次至少兩種值）、`S18`（註冊）、`S19`（金鑰回來零次 `PATCH`）、`S20`（500 仍進世界、無錯誤視窗、不重送）
- [ ] 2.5 `tests/avatar-picker.test.tsx`（既有）補 `S22`（選項數＝款數、色票＝軀幹色、選第八款送 7）；`tests/e2e/avatar-picker.mjs` 補 `S23`（1024 寬選擇器與八個選項都在視窗內）（Requirement〈選擇器列出每一款〉）
...
- [ ] 4.1 突變：`AVATAR_COUNT` 改回 2 → `S14`／`S22` 紅；某兩款同色 → `S14`＋`S16` 紅；隨機改成固定 0 → `S17` 的「至少兩種值」紅；金鑰路徑也 `PATCH` → `S19` 紅；失敗時重試 → `S20` 紅；`Math.floor(r × 8)` 寫死 8 → `S21` 的款數 3 那一半紅；拿掉 `flex-wrap` → `S23` 紅；`PATCH` 不 await 就導向 → `S17` 的順序紅
