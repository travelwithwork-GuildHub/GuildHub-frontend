...
## 3. 實作：看板面上的摘要與投影（feat/fe-w20-board-summary--render，含第 4 節四態）
- [x] 3.1 `BoardSummary`（DOM，Canvas 外）：兩塊看板各一組節點登記表（`Map<boardId, HTMLElement>`，不進 React state）；掛在 `WorldCanvas` overlay 層（`hall` 才掛）
- [x] 3.2 `BoardSummaryProjector`（Canvas 內，`useFrame`）：每塊看板算 `screenPixelFor(看板面錨點)`、寫 `translate3d`＋visibility、NaN 前置檢查、畫面外 `visibility:hidden`（移出無障礙樹）—— 照 `SeatAnchorProjector`；掛進 `SceneObjects`
- [x] 3.3 看板面錨點 y＝`BOARD_FACE_Y=1.45`（由 `semantic.tsx` 卡片/板面高度推導；出生點可辨識在 `S09` e2e 驗，必要時微調）
- [x] 3.4 卡片內容：專案＝標題、人才＝`display_name`；單行截斷（固定寬 `w-44`）；`min(4, n)` 張、其餘空槽（`S01`／`S02`）
- [x] 3.5 單元測試：兩筆→兩張填字卡＋兩空槽、剛好 4 筆無空槽、兩塊配對各畫各欄位（`S01`／`S02`）〔投影 `S03`／不進 React 併入 `S09` e2e 驗〕

## 4. 實作：四種狀態（併入 --render）
- [x] 4.1 載入中＝骨架卡（不先畫空位，`S06`）；空的＝`FE-X04`「這裡還沒有東西。」（走 `EMPTY_STATE_COPY` 匯出、不抄字串）、`role="status"`（`S04`）
- [x] 4.2 讀不到＝環境化錯誤：`role="status"`、**無 retry**、語彙 `FE-X03` `toUiError`、一律降級成 status（`S05`）
- [x] 4.3 stale：曾有資料後輪詢失敗保留舊的、不閃空白（`S05` 後半）
- [x] 4.4 一致性：兩邊 ready 對同一 page 0 時，看板標題＝面板前 4 筆同序（`S08`）—— 兩個真 hook 同掛比對
...
- [x] 6.2 `docs/adr/0015` 的「邊界狀態」改「已強制」、補證據路徑
