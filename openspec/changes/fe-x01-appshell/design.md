...
3. **`S04` 的重試按下去會不會真的恢復。** 測試可以斷言按鈕存在並被呼叫，
   但「重試之後內容真的載入了」需要一個真的網路層
4. **樣式 token 有沒有真的套用。** `S06` 只證明數值嚴格遞增，
   不證明畫面上的堆疊順序是對的

### 人工瀏覽器驗證：做什麼、留什麼

在 `feat/fe-x01-appshell--layout` 這一刀，起本機 dev server 之後逐條做，
**證據貼在那個 PR 上**：

| # | 動作 | 要留的證據 |
|---|---|---|
| V1 | `curl -sS -o /dev/null -w '%{http_code} %{redirect_url}\n' http://localhost:3000/` | 輸出逐字貼上，必須是 `307` 與 `.../world`。這是第 1 條缺口唯一的補法 |
| V2 | 瀏覽器開 `/world` | 截圖 ＋ console 訊息（必須沒有 error） |
| V3 | 瀏覽器開一個未定義路徑 | 截圖，必須是 404 而不是被轉去 `/world` |
| V4 | DevTools 阻斷 World 的 chunk 請求後重新載入 | 截圖：錯誤訊息與重試操作都在，**且頁面其餘部分仍在**（`S04` 的 `MUST NOT 整頁空白`） |
| V5 | 解除阻斷後按下重試 | 截圖：內容載入成功。這是第 3 條缺口的補法 |
| V6 | 檢視 `/world` 的伺服器端 HTML（`curl` 取原始碼） | 其中**不含** World 佔位元件的內容 —— 這證明第 2 條缺口的 `ssr: false` 真的生效 |

**連到哪裡**：只連本機 `localhost:3000` 自己起的 dev server。
