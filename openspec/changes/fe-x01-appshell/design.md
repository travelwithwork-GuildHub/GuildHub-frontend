...
**本 repo 對 web-facing 的 change 沒有成文的驗證規範，也沒有機器在驗。**
`AGENTS.md`〈完成的定義〉（第 481 行）只要求「有空狀態與錯誤狀態」與
「貼出實際輸出」，沒有規定瀏覽器怎麼驗。所以本 change 自己把方式定死在這裡，
證據形式一併寫明 —— 留白的話，實際發生的會是「我看起來沒問題」。

### 自動化涵蓋到哪裡

| Scenario | 怎麼證明 | 在哪一刀 |
|---|---|---|
| `S02` `S03` `S04` `S05` `S13` | Vitest ＋ Testing Library，直接 render 元件 | `--layout` |
| `S06` `S07` `S14` | Vitest 斷言常數表；`S07` 用 fixture ＋ 型別檢查非零 | `--layout` |
| `S08` `S09` | lint fixture：違規檔案使 `npm run lint` 非零 | `--scaffold` |
| `S10` `S11` `S12` | 四個指令的退出碼；`S11`／`S12` 各做一次負向驗證 | `--scaffold` |
| `S01` | **只證明到設定層**（見下） | `--layout` |

### 自動化證明不到的四件事

**這一節是誠實的缺口清單，不是免責聲明。**

1. **`S01` 的 307 只被證明到設定層。** D2 的測試斷言的是 `next.config` 的
   `redirects()` 回傳值裡 `permanent` 為 `false` —— **斷言設定不等於斷言回應**。
   設定寫對了，middleware、rewrite 或 Route Handler 照樣可以蓋掉它，
   而那個測試仍然是綠的
2. **client 邊界是否真的只在瀏覽器執行。** jsdom 裡沒有「伺服器端」這個概念，
   `ssr: false` 有沒有生效在 component test 裡看不出來
3. **`S04` 的失敗路徑在單元測試裡根本走不到。** 實測：`next/dynamic` 的
   載入失敗在 jsdom 裡**渲染空的、不拋錯**，錯誤邊界不會被觸發。
   所以單元測試只能直接給邊界一個會拋錯的 child —— 那證明的是**邊界本身的
   行為**（訊息、重試按鈕、按下去有沒有做事），不是「真的 chunk 失敗會走到
   這個邊界」。後者只有 V4／V5 證明得了。
   **邊界那一半一定要 import 正式碼**：在測試檔裡重刻一個 `catchError(...)`
   等於在測框架的 API，把正式碼的按鈕拔掉測試照樣是綠的（實作時踩過）
4. **樣式 token 有沒有真的套用。** `S06` 只證明數值嚴格遞增，
   不證明畫面上的堆疊順序是對的

### 人工瀏覽器驗證：做什麼、留什麼

**⚠️ 必須在 production build 上做（`npm run build` ＋ `next start`），不是 dev server。**

這一條是 2026-09-07 實作時實測改的，原本寫的是 dev server。
**V4／V5 在 dev server 上驗不出來**：`next/dynamic` 的 chunk 請求失敗時，
dev server 的載入 promise **永久 pending**（實測 12 秒後仍然沒有任何
rejection），畫面永久空白，錯誤邊界根本不會被觸發。jsdom 也是同一個行為
（渲染空的、不拋錯）。只有 production build 會真的拋錯讓邊界接住。

也就是說：**在 dev 上驗 V4，看到的空白畫面會被誤讀成「規格沒實作」；
在 dev 上驗 V5，永遠等不到那個按鈕。** 兩者都會得到錯的結論。

在實作那幾刀跑完，**證據貼在對應的 PR 上**：

| # | 動作 | 要留的證據 |
|---|---|---|
| V1 | `curl -sS -o /dev/null -w '%{http_code} %{redirect_url}\n' http://localhost:3001/` | 輸出逐字貼上，必須是 `307` 與 `.../world`。這是第 1 條缺口唯一的補法 |
| V2 | 瀏覽器開 `/world` | 截圖 ＋ console 訊息（必須沒有 error） |
| V3 | 瀏覽器開一個未定義路徑 | 截圖，必須是 404 而不是被轉去 `/world` |
| V4 | 阻斷 World 的 chunk 請求後重新載入 | 截圖：錯誤訊息與重試操作都在，**且頁面其餘部分仍在**（`S04` 的 `MUST NOT 整頁空白`） |
| V5 | 解除阻斷後按下重試 | 截圖：內容載入成功。這是第 3 條缺口的補法 |
| V6 | 檢視 `/world` 的伺服器端 HTML（`curl` 取原始碼） | 其中**不含** World 佔位元件的內容 —— 這證明第 2 條缺口的 `ssr: false` 真的生效 |

**V4 要擋的是哪一個 chunk：用內容找，不要用行為猜。** 檔名是內容雜湊，
每次 build 都會變；逐一阻斷再看「哪一個會讓畫面壞掉」是不可靠的
（實測兩次得到不同答案）。正確做法是抓回每個 chunk 的內容，
找含 World 佔位元件識別字串的那一個。

**選擇器要限定範圍。** Next 有一個隱藏的 `__next-route-announcer__`，
它也是 `role="alert"` 而且排在前面 —— 用 `[role="alert"]` 會靜靜抓到它，
然後把「功能正常」誤判成「功能壞掉」（實測踩過）。用 `main [role="alert"]`。

**連到哪裡**：只連本機自己起的實例（`npm run build` ＋ `next start`）。
不連任何共用實例（`AGENTS.md`〈測試環境隔離〉）。

**這個規格 PR 本身沒有瀏覽器證據，因為還沒有任何程式碼。**
V1–V6 屬於實作 PR 的完成條件，列在 `tasks.md` 的第 6 組。

### 為什麼不現在就自動化這六條

V1–V6 全部可以用瀏覽器自動化工具跑起來，而**那正是 `FE-O11`（測試策略，W1）
要裁決的東西**：E2E 用什麼、佔多少比重、3D 哪些值得 E2E。
在它之前先選一套工具進 CI，等於替它決定了。

所以本次的取捨是：**先把「要驗什麼」定死（就是上面那張表），
「誰來跑」留給 `FE-O11`。** 定義比工具先固定，之後換工具不必重新想一次要驗什麼。
...
不能靠 component test 完整證明。緩解：S01 斷言 `next.config` 的轉址設定
（見 D2，**只到設定層**），S03 讓頁面元件保持同步以便直接 render，
S02 直接 render not-found 元件。
**剩下的四個缺口與各自的補法列在〈驗證方式〉，不是含糊帶過。**

**R6｜人工驗證不會在下一個 PR 重跑** → 〈驗證方式〉的 V1–V6 是人跑的，
所以它只證明「合併當下是對的」，之後任何一次改動都可能悄悄弄壞它而沒有人發現。
這是本次接受的代價，不是被忽略的問題。
緩解：把 V1–V6 寫成一張具名的表，讓 `FE-O11` 決定工具之後可以逐條轉成自動化，
不必重新推導要驗什麼。**這個風險在 `FE-O11` 落地前一直存在。**
