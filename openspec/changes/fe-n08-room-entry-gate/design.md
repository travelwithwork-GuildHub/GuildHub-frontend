...
晚到的成功不存票、不 `enterRoom`、不重開視窗，晚到的失敗不顯示。作廢的條件有三個、各自獨立：視窗關過、目標房間換了、
**回應到達時的身分 ≠ 送出時的身分**（換人或登出；視窗本身不關 —— 訪客本來就能開視窗，D2）。S15 的第二段刻意不關視窗，才驗得到第三個。不用 `AbortSignal`：取消 fetch 不代表後端沒簽票。
