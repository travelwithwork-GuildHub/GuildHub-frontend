...
- 送出中能不能 Esc／關閉？本 change 定為**不能**（busy 期間 Esc 與關閉無效，請求結束才恢復）——
  避免「關了之後晚到的成功突然把人送進房」這種競態；`enterProject()` 也就不需要 `AbortSignal`。
  第一次 review 時若認為體驗太硬，改成可取消要另補「晚到結果一律丟棄」的 Scenario。
