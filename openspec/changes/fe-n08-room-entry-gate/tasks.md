...
- [ ] 5.1 先寫 jsdom：`[FE-N08-S11]`（被拒後票還在；不按就同票再試；按了才 drop、確認不在了才關通知開空視窗、可及名稱含房名；啟動前沒有 `/enter`；drop 三種故障 → 不開視窗、通知留著；深連結沒 title → 無房名的視窗，清單回來後有）；`FE-V01-S07`／`S11` 的既有測試不動（新條件由 `FE-N08-S11`／`S01` 自己驗）
