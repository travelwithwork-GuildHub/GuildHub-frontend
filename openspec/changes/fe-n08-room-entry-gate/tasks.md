...
- [ ] 2.1 先寫 `tests/contract/rest/enter.contract.ts`：`[FE-N08-S12]` 的矩陣（401、404、404、403、200、closed 有密碼 200；名片已刪那列是已知差異，不進矩陣）、座位空與滿都 200、票對替身握手（同房同人收、別房拒、別人的 cookie 拒）、`[FE-N08-S13]` 兩個目標同一份 —— 對 `local` 目標先紅
...
- [ ] 2.3 `src/app/api/projects/[project_id]/enter/route.ts`：走 `handle()` 管線（沒有 session → 401；它另外查名片存在，那是跟真後端的已知差異）、查 `password_hash`（不看 status）→ 404、`verifyPassword` → 403、簽票（綁 projectId＋profileId）
