...
權限：**不適用** —— 兩個端點都只要求已登入；未登入的呈現已由 `FE-X04` 的權限阻擋處理
併發：**適用** —— 快速連點不同人才時，詳情的回應會交錯到達
持久資料相容性：**不適用** —— 只讀，不寫任何持久資料
失敗路徑：**適用** —— 詳情載入失敗、缺值（`hours_per_week`／`bio` 為 `null`）各有正確行為

測試連到什麼：**不連任何團隊共用的位址。**
元件判準直接掛載；接線判準走 `tests/support/contract-server.ts`（本機起的微型 HTTP server）。
