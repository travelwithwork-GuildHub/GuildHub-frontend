...
權限：不適用 —— 不做授權判斷
併發：不適用 —— 只有一個本地玩家，沒有共享可變狀態
持久資料相容性：不適用 —— 不讀寫任何持久資料
失敗路徑：適用 —— 非有限的時間間隔、同時按下相反方向鍵
測試環境：**不適用 —— 測試不連任何外部服務。** 輸入、移動與動畫都是純函式；
角色的視覺由人工在 production build 上確認

**⚠️ 架構約束**：位置、朝向與動畫相位 MUST NOT 寫入 React state 或 Zustand
（`CONTEXT.md`「高頻資料不進 React」）。違反不會有錯誤訊息，只會變慢。
