...
權限：不適用 —— 不做授權判斷
併發：不適用 —— 物理步進只在 render loop 裡發生
持久資料相容性：不適用 —— 不讀寫任何持久資料
失敗路徑：適用 —— 走向障礙物、走向邊界、以極高速度移動
測試環境：**不適用 —— 測試不連任何外部服務。** 物理世界在本機行程內
建立與步進，不開網路連線，不需要瀏覽器

**⚠️ 架構約束**：角色位置的權威來源是 **Rapier rigid body**，
不是 React state（`CONTEXT.md` 明訂 rigid body 是允許放高頻資料的地方）。
