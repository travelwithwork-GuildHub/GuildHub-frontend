...
權限：**不適用** —— 兩個端點都只要求已登入，這一列不做任何授權判斷
併發：**適用，而且是這一份的重點之一** —— 翻頁的請求會重疊，回應的到達順序不保證
持久資料相容性：**不適用** —— 只讀，不寫任何持久資料
失敗路徑：**適用** —— 請求失敗、翻到底、首次無資料三者的正確行為各不相同

測試連到什麼：**不連任何團隊共用的位址。**
單元判準完全不連外部服務（攔在 `src/api/` 這一層）；端到端判準只打
**本機自己起的** dev server，並在瀏覽器層攔截 `/api/*`。
