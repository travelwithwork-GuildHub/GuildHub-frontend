...
它只呈現 `scene-chat-transport` 交出的東西（目前頁面、目前場景、最多 100 筆），不暗示有持久歷史、私聊、未讀或跨場景保存；
使用者提供的字串只當文字。

## Applicability

權限：不適用 —— 大廳可匿名；房間進不進得去由既有握手決定。
併發：適用 —— 新訊息、使用者捲動、送出、場景過場會同時發生。
持久資料相容性：不適用 —— 不讀寫 DB、不用 Web Storage。
失敗路徑：適用 —— 全空白、沒連線、沒 `ready`、transport 拋錯、過場失敗、舊場景晚到。
名詞：「committed 場景」與「記憶體」是 `scene-chat-transport` 的（`useSceneChat().log`）；本文不重述它的清空與歸屬規則，只驗它在畫面上的可觀察形式。
測試連線：jsdom 不連任何服務（`send` 由假的 transport 接）；e2e 只打本機 `next start` 的 loopback，WebSocket 用 `routeWebSocket` 偽造、REST 用 `page.route`，MUST NOT 連任何團隊共用位址。
...
Escape MUST NOT 關掉 chat 區、MUST NOT 導覽。chat 區 MUST NOT 遮住互動提示（`InteractionPrompt`）。
...
記憶體標記 `truncated` 的那一列 SHALL 讓使用者辨識它被截斷過（怎麼標是 UI 的字）；沒標的 MUST NOT 有那個標記。
記憶體為空時 chat 區 SHALL 有可辨識的空狀態，且 SHALL 沒有任何訊息列；空狀態 MUST NOT 宣稱伺服器沒有歷史、MUST NOT 宣稱這個場景從沒有人發言
（它只代表「這一頁目前還沒收到」）。

> 拔掉什麼會紅：用 `Map`／排序改了順序 → S03；`truncated` 不標或全標 → S03；空狀態寫「沒有歷史」 → S04；列表用 `innerHTML` → `output-safety` 的 S14 與既有 lint。

#### Scenario: [FE-K04-S03] 三則依序、各自認得出誰講的；截斷的那一則有標記

- **WHEN** 依序收到 阿福「一」、小美「二」、阿福「三」，接著一則 `body` 是 2001 個 code point 的
- **THEN** 列表 SHALL 依「一」「二」「三」排列，每一列的 `name` 節點 SHALL 分別是「阿福」「小美」「阿福」
- **AND** 第四列 SHALL 有截斷標記、其 `body` 節點是前 2000 個 code point；前三列 SHALL 沒有截斷標記
- → 驗於：jsdom

#### Scenario: [FE-K04-S04] 還沒收到任何訊息：空狀態，不下結論

- **WHEN** 記憶體是空的
- **THEN** chat 區 SHALL 有空狀態、SHALL 沒有訊息列
- **AND** 空狀態的文字 MUST NOT 含「歷史」「沒有人」「從未」
- → 驗於：jsdom
...
`send` 同步回來沒拋 → 輸入框 SHALL 清空；`send` 拋（沒連線、沒 `ready`、socket 拋）→ 輸入框 SHALL 保留原值、送出控制之前 SHALL 恰好一個
`role="alert"` 讓使用者辨識沒送出去、MUST NOT 自動重送；那句 MUST NOT 含後端字串或例外訊息。回聲前列表 MUST NOT 出現自己剛送的話。
送出 SHALL 用 Enter 或送出控制；Shift+Enter SHALL 換行（多行輸入）。

> 拔掉什麼會紅：送出前 trim → S05 的「原字串」；不擋全空白 → S05；`maxlength` → S07；沒 ready 也清空 → S06；`send` 拋錯被吞 → S06 的 alert；送出時本地 append → S06 的「回聲前沒有」。
...
#### Scenario: [FE-K04-S06] 沒 ready 送：保留、有 alert；ready 後送：清空、回聲前列表沒有

- **GIVEN** transport 的 `send` 會拋 `RealtimeError`（連線沒 ready）
- **WHEN** 輸入「哈囉」送出
- **THEN** 輸入框 SHALL 仍是「哈囉」；送出控制之前 SHALL 恰好一個 `role="alert"`、焦點 SHALL 在它上面；列表 SHALL 沒有「哈囉」；SHALL 沒有第二次 `send`
...
- **AND** 整段期間 `page.on('request')` 收到的請求 SHALL 只有 `/api/me`、`/api/rooms`、`/api/profiles`／`/api/projects` 的清單、Next 的靜態資源與 `/ws`
- → 驗於：e2e（這條同時是 `FE-R11-S05` 的可觀察形式；`tests/e2e/scene-chat.mjs` 正式引用兩個 ID）

### Requirement: 新訊息不打斷正在讀舊訊息的人

使用者在列表底部時收到新訊息，最新的一則 SHALL 在可見區裡。使用者已往上捲、最後一則不在可見區時收到新訊息，捲動位置 MUST NOT 被移到底；
chat 區 SHALL 讓使用者辨識有新訊息、並提供一個回到最新的控制，啟動它後最新的一則 SHALL 在可見區。
...
- **WHEN** 新訊息到達
- **THEN** 捲動位置 SHALL 不變（`scrollTop` 差 ≤ 1px）；SHALL 出現「有新訊息」的可辨識控制
- **AND WHEN** 啟動那個控制
