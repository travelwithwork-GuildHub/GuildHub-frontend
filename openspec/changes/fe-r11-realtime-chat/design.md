...
**巨大訊息**：兩位審查者意見相反 —— 一位說前端擋會掩蓋後端缺口（`BE-G16`），一位說客戶端資源防禦與後端契約是兩件事、100 筆 × 無上限＝理論上 1 GB。
兩邊都對一半，所以分開：**契約層**不擋（`LIMITS` 如實記 UNBOUNDED、`ChatIn`／`ChatOut` 不加長度檢查、驗證器對合法訊息一律回成功、不拒收不丟棄）；
**保存層**有自己的預算（單則超過 2000 code point 只留前 2000、標 `truncated`，`FE-K04` 讓人看得出來）——它是客戶端的資源政策，
不寫進 `LIMITS`、不假裝是後端限制、不取代 `BE-G16`（後端仍該定大小上限與 rate limit）。2000 取 Inbox 的既有上限（同一個「一則訊息多長算合理」的產品直覺），
是產品決定，進 Requirement。`FE-O07` 銜接清單列 `BE-G16`。
