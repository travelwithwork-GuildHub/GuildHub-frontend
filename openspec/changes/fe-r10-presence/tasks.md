...
- [ ] 3.1 先為 `FE-R10-S07`／`S08` 寫失敗測試，證明初始人數包含自己，且只有有效 join／leave 會改變人數
- [ ] 3.2 以 snapshot-ready 與遠端 roster 推導 `roster.size + 1`，讓 `FE-R10-S07`／`S08` 通過，不新增獨立累加器
- [ ] 3.3 先為 `FE-R10-S09` 寫失敗測試，涵蓋卸載、換連線以及新 snapshot 到達前不顯示舊值
...
- [ ] 5.1 執行 lint、typecheck、完整單元測試與適用的 integration／E2E；記錄通過數量，並逐條對照 `FE-R10-S01`–`S10` 都有非恆真的證據
