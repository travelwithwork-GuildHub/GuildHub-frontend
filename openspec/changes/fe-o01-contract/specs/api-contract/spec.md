...
權限：不適用 —— 這一刀不做授權判斷。錯誤 envelope 只描述線路上的形狀，
處置（401 導向登入之類）屬於 `FE-X03`。
併發：不適用 —— 純資料定義，沒有共享狀態、沒有時序。
持久資料相容性：不適用 —— 不讀寫任何持久資料。
失敗路徑：適用 —— 超長、缺必填、型別不符、未知訊息類型、浮點座標、朝向越界。
**測試連到什麼**：不適用 —— 測試不連任何外部服務，全部是純資料的 parse 與型別檢查。
（產生型別哨兵的檔案需要一份跑著的後端，但那是開發者在自己機器上跑一次的
產生步驟，不是測試的一部分 —— 見 `design.md`。）
...
### Requirement: 錯誤 envelope 描述線路上的形狀

系統 SHALL 定義後端錯誤回應的形狀為 `{ detail: <字串> | <驗證錯誤陣列> }`，
兩種形狀都要涵蓋 —— 後端自己丟的錯誤 `detail` 是可直接顯示的中文字串，
而框架自動產生的 422 的 `detail` 是結構化錯誤陣列。

系統 SHALL 記錄後端實際會回的 status code 集合：
`400`、`401`、`403`、`404`、`409`、`422`、`500`。
這個集合 MUST NOT 從 OpenAPI 產生 —— 後端的 OpenAPI **只宣告 `200`/`201`/`422`**，
其餘四個碼在產出的型別裡完全不存在。

契約 MUST NOT 定義任何處置（要不要導向登入、要不要重拉列表）。
那是 `FE-X03` 的唯一一份錯誤語彙。

#### Scenario: [FE-O01-S09] 兩種 detail 形狀都驗得過
...
#### Scenario: [FE-O01-S10] 產出的型別與契約推導的型別一致

- **WHEN** 對版控中的產出型別檔跑 typecheck
- **THEN** 通過，且每一個 REST 實體都有一條相等斷言涵蓋它
- **AND** 沒有任何實體只有單向的相容性斷言
