...
## 7b. 撞到 `FE-W07-S06`（新的 GPU 資源建立者必須登記進洩漏偵測）

兩個模型都選「登記進去」，一致否決「改規格排除」與「把 cache 移出 `src/world/`」。

- [x] 7b.1 登記進 `SUBJECTS`，`expect: 'clean'`，wrapper 設 `dispose={null}`
- [x] 7b.2 負向驗證：拿掉 cache → 真瀏覽器每輪 +1 個 geometry，兩個 subject 變紅
- [x] 7b.3 量測台檔頭寫明**只證明得了 geometry 那一半**（`info.memory` 無 material 欄位）
