...
權限：不適用 —— 身分由後端從 session cookie 解析，這一層不做判斷。
併發：適用 —— `hello` 與 `snapshot` 相隔不到 1 毫秒到達，監聽器的安裝時機
會決定拿不拿得到自己的 id。
持久資料相容性：不適用 —— 即時層的資料不落 DB。
失敗路徑：適用 —— 握手被拒、連線中途斷掉、在還不能送的時候送、重複關閉。
**測試連到什麼**：單元測試**不連任何外部服務**（用替身 WebSocket）。
另有一組整合驗證要連**開發者自己 `bash run.sh` 起的那一份後端**，
**不進 CI**（`AGENTS.md`：CI 不提供任何服務）—— 見 `design.md` 的驗證方式。
