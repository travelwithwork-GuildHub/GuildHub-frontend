...
權限：不適用 —— 進房間的授權在後端握手（`manager.py:71-79`），前端只帶票、不判斷。
併發：適用 —— 舊連線的關閉與新連線的建立、過場逾時計時器與 `ready`／`closed` 事件同時發生；舊過場的事件遲到；React Strict Mode 的雙重 effect。
持久資料相容性：不適用 —— `sessionStorage` 的票是可拋棄的；沒有就回大廳。
失敗路徑：適用 —— 握手被拒（客戶端分不出原因）、10 秒沒 `ready`、回大廳也連不上、`projectId` 不是 uuid、沒有票、第二個分頁、舊過場的事件遲到。
測試連到什麼：單元測試注入假 socket ＋ 假時鐘、不連任何服務；瀏覽器測試只打本機自己起的 dev server，`/ws` 與 `GET /api/rooms` 都偽造（`page.routeWebSocket`／`page.route`）。不連任何團隊共用的位址。
