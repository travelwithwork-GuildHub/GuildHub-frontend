...
- [ ] 5.1 `ListPanel` 的 Tab 循環；世界焦點錨（`data-focus-anchor="world"`）；`closePanel` 回錨；詳情關閉回那張卡
- [ ] 5.2 判準：`S12`（jsdom）、`S11`、`S13`（Playwright）
- [ ] 5.3 **突變**：關閉不回錨，`S13` 要紅
...
- [ ] 6.1 Escape、Tab 走離即關、關閉回按鈕；不 hold 鎖
- [ ] 6.2 判準：`S14`、`S15`、`S16`、`S17`
- [ ] 6.3 **突變**：拿掉 Tab 走離即關，`S16`／`S17` 要紅；picker hold 鎖，`S14` 要紅

## 7. 收尾

- [ ] 7.1 `npm run typecheck`、`npm run lint`、`npm test` 全綠
- [ ] 7.2 Playwright：`S11`、`S13`，以及 Escape 兩層各一次；截圖進 `docs/evidence/fe-x06/`
