...
權限：**不適用** —— 鍵盤與焦點的治理跟誰登入無關
併發：**適用** —— 多個鎖的持有者會交錯地取得與釋放；焦點在同一個事件迴圈裡從一個控制移到另一個
持久資料相容性：**不適用** —— 不讀寫持久資料
失敗路徑：**適用** —— 鎖沒放、焦點掉到 `body`、Escape 一次關兩層，每一種都是「畫面上看不出來」的壞

測試連到什麼：**不連任何團隊共用的位址。**
單元判準用測試用的 `<input>` 與真的 `LocalPlayer`（three 的 renderer）；
focus trap 與真的 Tab／Escape 在 Playwright 打**本機自己起的** dev server。
