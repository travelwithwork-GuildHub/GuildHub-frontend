...
缺任何一件 SHALL 是配置錯誤；識別字 SHALL 含該索引且全配置唯一。排列 SHALL 是由索引決定的（同一索引永遠同一位置）：0–3 在通道西側、4–7 在東側，各自**由南（門口）到北**（z 隨索引遞減；南是 +z），
所以 0 與 4 是離門口最近的一對；站位 SHALL 朝向通道（站位在桌子與通道之間）。本能力 MUST NOT 讀 `seat_count`。
...
所以收斂後「角色相對某個錨點的位置」＝畫面中心到那個錨點的向量；換算世界距離：x 每單位 s px（用兩個已知 x 距離的錨點量出）、z 每單位 s/√2 px（俯角 45°）。
...
- **AND WHEN** 相機放在出生點的固定偏移、viewport 是 1280×720（jsdom 裡用投影函式算）
- **THEN** 投影落在畫面外的錨點（北端那幾個）SHALL 是 hidden，畫面內的 SHALL 不是 hidden
- **AND WHEN** 場景是 `hall`
- **THEN** SHALL 一個都沒有
- → 驗於：jsdom
...
- **AND** 桌子有畫出來：對每個在畫面內的錨點，取以它為中心、邊長 s×0.4 px 的方塊（桌面投影的內側，不含桌腳與椅子）裡的像素，
  與同一畫面上通道中線的地板取樣的顏色距離（RGB 歐氏）> 40 的像素比例 SHALL ≥ 60%
  （突變：把桌面的 mesh 拿掉、桌腳、地毯與椅子留著 → 那個方塊露出的是地板 → 比例掉到 60% 以下 → 紅；40／60%／0.4 三個數字要在正式建置上跑 3 次、
  含突變各 3 次確認穩定，不穩就重開 spec PR 調）
...
- **THEN** 走到站位；接著直接朝它的桌子走：SHALL 先有連續 3 步每步位移 ≥ 2 px（證明輸入有生效、步幅高於容差），
  角色 SHALL 接近到與該錨點的反算距離 < 2 單位，之後連續 5 步距離的變化都 ≤ 1 px（撞到桌子的 plateau）；步數上限 60
