...
所以 0 與 4 是離門口最近的一對；站位 SHALL 朝向通道（站位在桌子與通道之間）。（「位置由一個純函式推導」是 design 的實作決定，不是判準 —— 手寫八筆只要符合這些約束也合規。）本能力 MUST NOT 讀 `seat_count`。
...
- **THEN** 走到站位；接著直接朝它的桌子走（以下距離全部在螢幕空間量，單位 px；「單位」指世界單位、以 s 換算）：
  SHALL 先有連續 3 步每步位移 ≥ 2 px（證明輸入有生效、步幅高於容差），角色 SHALL 接近到與該錨點的螢幕距離 < 2·s px，
