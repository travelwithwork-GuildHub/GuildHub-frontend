...
- **AND** 桌子有畫出來：對每個在畫面內的錨點，取以它為中心、邊長 `DESK_SAMPLE_SIDE`（以 s 為單位）px 的方塊（桌面投影的內側，不含桌腳與椅子）裡的像素，
  ...
  量到的分布：正常建置 3 次 × 8 張桌全部 100%；拔桌面 mesh 3 次的最大值 18.8%（門檻 20）／17.0%（門檻 40）——
  非零的部分是留下來的桌腳投在方塊裡的陰影，不是桌面 —— 間隔 81 個百分點，門檻取中間。
  裸地板的基準是**一個畫面一份**（地板是單一材質、量到的顏色離散 0）：取樣點在畫面外的錨點用同一畫面上別張桌子旁的取樣
...
- **THEN** 走到站位；接著直接朝它的桌子走（以下距離全部在螢幕空間量，單位 px；「單位」指世界單位、以 s 換算）：
  SHALL 先有連續 3 步每步位移 ≥ 2 px（證明輸入有生效、步幅高於容差），角色 SHALL 接近到與該錨點（扣掉桌面高度偏移後）的反算距離 < 2 單位，
  之後連續 5 步該距離的變化都 ≤ 1 px（plateau）；plateau 時的反算距離 SHALL 等於「桌子碰撞盒近側邊到桌面中心的距離 ＋ 角色半徑」
