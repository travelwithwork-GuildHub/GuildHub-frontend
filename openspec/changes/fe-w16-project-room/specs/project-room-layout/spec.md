...
房間場景 SHALL 為每個工位提供一個可從 DOM 查到的投影位置（`data-seat-index` 0–7 各一）：元素的**中心** SHALL 每 frame 對齊「桌面中心」
（世界座標 x、z 取桌子的中心，y 取桌面高度）的螢幕投影（門標籤的同一套投影函式，但對齊點是中心不是左上角）；
座標 SHALL 是有限數（不是 NaN），投影落在畫面外時元素 SHALL 標成 hidden 但仍存在。今天沒有可見內容（`aria-hidden`）。
它只在 `room` 場景存在；在 `hall` MUST NOT 存在。本地角色永遠在畫面正中央（相機跟著角色、正交投影；`scene-switch.mjs` 的里程計就靠這點），
所以「角色相對某個錨點的位置」＝畫面中心到那個錨點的向量。
...
- **AND** 桌子有畫出來：對每個在畫面內的錨點，取以它為中心、邊長 s×0.4 px 的方塊裡的像素，與同一畫面上通道中線的地板取樣的顏色距離（RGB 歐氏）> 40 的像素比例 SHALL ≥ 60%
  （突變：把桌面的 mesh 拿掉、地毯與椅子留著 → 那個方塊露出的是地板 → 比例掉到 60% 以下 → 紅）
...
- **THEN** SHALL 先有連續 3 步每步位移 ≥ 2 px（證明輸入有生效、步幅高於容差），角色 SHALL 接近到與該錨點的距離 < 2 單位，之後連續 5 步距離的變化都 ≤ 1 px（撞到桌子的 plateau）；步數上限 60
