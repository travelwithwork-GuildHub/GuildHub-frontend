...
**真後端沒有這個保證**：它的票全放在同一個 session cookie，並行 `enter` 會互相覆寫（2026-09-17 實測同一個 cookie jar 並行 enter 兩間是 `403`／`200`）。
所以「並行 enter 兩間都拿得到票」只寫在本地獨有的 `FE-J14-S37`，兩個目標共用的 `FE-J14-S34` 只驗循序；這是已知差異，`enter` 是凍結端點，不改真後端。
