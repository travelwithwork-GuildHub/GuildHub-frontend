...
> ⚠️ **這一節送出三個 PR**（理由同第 6 節）：整節一個 PR 的話遠超 `AGENTS.md`〈PR 的大小〉的 250 行產品碼
>（`--form` 一支自己就量到 334，被迫先切一刀）。按 Scenario 切，每一片**各自帶著證明它的判準**：
>
> | 片 | Scenario | 判準 | 產品碼 |
> |---|---|---|---|
> | `--form` | `S11`～`S15`、`S21` 的表單那半、`S36` 的寫入那半（＋`FE-X16-S14` 的讓位協定） | `tests/resources-form.test.tsx` | 已合併（#627） |
> | `--edit` | `S17`、`S18`、`S19`、`S07`、`S08`、〈寫入控制項只給寫入者〉最後一句（寫入 403 ＋ active） | `tests/resources-edit.test.tsx` | 159 行 |
> | `--delete` | `S20`、`S35` 的確認層那段、`S16`、`S07` 的「按刪除」那個起點、`S08` 最後一句（滿了就不能再新增） | `tests/resources-delete.test.tsx` | 204 行 |
>
> **寫入失敗之後的那一次專案狀態確認（D2）整條放在 `--edit`**：新增、修改、刪除共用同一條路徑，
> 拆開會讓不同 PR 各拿到它的一半。所以 `--form` 合併後、`--edit` 合併前，新增的 403／409
> 只會走 `FE-X05` 的預設失敗呈現（不確認、不會說「已結案」）—— **這是已知的缺口，不是漏做**。
> `--edit` 合併後那個缺口補上了，`--delete` 合併後**第 7 節就沒有已知缺口了**
>（原本記的兩個 ——「刪除按下去不做事」與「50 筆時新增鈕仍按得下去」—— 都在 `--delete` 補上）。
>
> **三片都合併之後才算數**：checkbox 勾在 `--delete` 上，但 7.1／7.2／7.3 描述的是整節的成果。
>
> `--delete` 的突變表（18 個，16 紅）在 `TWW/FE-J14-delete-PR說明.md`。
> 其中「`store.removed()` 不推進 `seq`」那一個是**外部審查抓出來的缺口**：原本 17 個突變裡沒有它，
> 拿掉那一行五個 resources 測試檔全綠。補了 `S36` 刪除那一半的 store 層判準之後才紅。
>
> ⚠️ **`--delete` 沒有做、而且刻意不做的兩件事**（不是漏做）：
> - **刪除的 404「先移除那一列」沒有判準壓著**。規格同時要求「移除」與「重讀一次」，而重讀會把清單
>   整份換掉 —— 突變把 `store.removed` 那一行拿掉，十條判準仍然全綠。那一行照規格留著，
>   但它的載重來自 204 那條路徑，不是 404。
> - **上限滿了之後 handler 裡的 `if (full) return` 沒有判準壓著**。React 不會把 click 派送給
>   `disabled` 的鈕，所以「對 disabled 的鈕發 click」在 jsdom 裡量不到那一行
>   （`25b2e97` 留下的那句註解說量得到，**那是錯的**，已改掉）。擋下 `POST` 的是 `disabled` 本身。

- [x] 7.1 先寫 jsdom：`[FE-J14-S07]`、`[S08]`、`[S11]`～`[S20]`，**以及第 6 節延後的那三個半條**：`[S21]` 的「在欄位裡打 wasde 角色不動」與「Escape 先關表單、鎖不放」、`[S35]` 的刪除確認層那段、`[S36]` 的「新增成功後晚到的讀取不得覆蓋」那段
- [x] 7.2 實作新增／修改表單（`useForm`；名稱只含空白是送出時錯誤；網址 `safeHref` ∧ 無空白是即時錯誤，design D5）、刪除確認層、上限、可見範圍說明
- [x] 7.3 突變：送 `safeHref` 正規化值 → S11 紅；拿掉「含空白」判定 → S13 紅；`LIMITS` 或上限常數寫死 → S12／S16 的 mock 那段紅；PATCH 送全部欄位 → S17 紅；沒改也送 → S18 紅；409 不重讀清單 → S08 紅；樂觀更新 → S11 的「201 之前仍是兩筆」紅；確認層不防連按 → S20 紅；表單開著時 Escape 直接關掉面板 → `S21` 的表單那半紅；確認層自己組 `<a href>` → `S35` 紅；寫入成功後不丟棄晚到的讀取 → `S36` 紅
