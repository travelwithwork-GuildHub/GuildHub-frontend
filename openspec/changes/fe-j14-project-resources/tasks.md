...
> | `--delete` | `S20`、`S35` 的確認層那段、`S16`、`S07` 的「按刪除」那個起點、`S08` 最後一句（滿了就不能再新增） | 那一支自己的判準檔 | 待做 |
>
...
> `--edit` 合併後那個缺口補上了，剩下的已知缺口只有兩個：**刪除按下去仍然不做事**、
> **50 筆時新增鈕仍然按得下去**（`S16` 的上限與 `S08` 最後一句都要刪除才量得到「刪一筆就恢復」）。
> 7.1、7.2、7.3 的 checkbox 要等 `--delete` 才勾得起來。
