...
元件怎麼讀寫資料。**元件裡不准出現 `fetch`** —— 散在各處的 `fetch` 讓
「之後接真後端」變成重寫不是切換，而 `FE-B`／`FE-J`／`FE-M`／`FE-N` 四組
合計五十幾項都要讀寫資料。

這一層做三件事：把「位址從哪來、憑證怎麼帶」收斂到一個地方；
讓資料來源可以由設定切換，而**呼叫端不知道自己連的是誰**；
把契約的檢查放在 API 邊界 —— 送出去之前驗輸入，回來之後驗結構。

邊界的兩個方向刻意不對稱：後端**新增**欄位要通過（那是相容的變更），
**移除或改型別**要立刻炸（讓它流下去的話，症狀是深層元件裡的
`undefined is not an object`，跟真正的原因隔了很遠）。

範圍只有真後端今天有的端點。後端沒有的 domain 留白，並用機械檢查擋住 ——
