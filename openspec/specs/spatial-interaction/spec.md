...
空間裡「我現在對著誰、離它多遠」的判定。它是**機制層**，不是產品操作 ——
物件長什麼樣、按下去打開什麼，分別由 `FE-W12` 與各個產品項目負責。

`CONTEXT.md` 對這一層下了一句警語：「如果主要行為都是走到一個物件、按 E、
打開 DOM 面板，那 3D 就是一條很貴的導覽列。」所以這份規格刻意**不**把
「按 E 開面板」寫死成唯一的互動形狀：contract 交出的是 active target
**與連續的距離**，而不是一個 boolean —— `FE-V04` 漸進式接近要的是
「越靠近顯示越多、每一步都能退回」。

三個使用者看得到的失敗由這裡擋住：提示在兩個物件之間閃爍、
沒有目標時距離被誤讀成「貼在旁邊」、兩個物件共用 id 造成「有時候按了沒反應」。
