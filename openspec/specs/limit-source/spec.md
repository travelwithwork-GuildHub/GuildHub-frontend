# limit-source Specification

## Purpose
長度與範圍的數字只有一個家：`src/api/contract/limits.ts` 的 `LIMITS`。契約 schema 的 `.min()`／`.max()` 只能接它
（一條窄的 lint 守著），契約測試的成對邊界從它算，每一個數字都記著從哪抄的、什麼時候對過。

長度單位是 Unicode code point —— 後端的 `char_length` 與 Python 的 `len` 都這樣數，而 JS 的 `.length` 與 HTML `maxlength`
數的是 UTF-16 code unit，20 個 emoji 會差一倍。所以算法也只有一份（`codePointLength`／`remaining`／`violates`），
表單不用原生 `maxlength`。登入表單的暱稱欄是第一個真的拿到這些數字的 UI：剩餘字數、超過上限就不能送。
