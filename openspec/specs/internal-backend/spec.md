# internal-backend Specification

## Purpose
前端自己的後端：Next.js Route Handlers 讀寫 `disposable-db` 那個可拋棄的 Postgres，讓 W6–W12 的產品能力不必等真後端。
它**刻意複製真後端的可觀察行為與怪癖**：同一條請求管線、同一種錯誤形狀（401 `未登入`、404 中文 detail、
422 Pydantic 形狀、資料庫 check 違反是 500 text/plain 而不是 422）、分頁 20 筆 0-based 沒有 total、
login 三種模式剛好一組、簽章的 HttpOnly session cookie。本地版比真後端「好用」就是在製造假象。

即時層的替身是另一個程序，訊息形狀重用 `src/api/contract/ws.ts`，怪癖（靜止不送 pos、不合協定靜默丟、
自己的 move 廣播回自己、握手失敗不給 err）一併複製。`FE-O05` 的契約測試對本地版與真後端跑同一組，是唯一能防止它漂走的東西。
