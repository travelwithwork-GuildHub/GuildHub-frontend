# disposable-db Specification

## Purpose
前端自己的後端（Route Handlers）讀寫的那個資料庫：一個**可拋棄的 Postgres**。它的 schema 與 seed 是後端 `sql/` 檔案的逐字複本，
後端 repo 在本機時由機器逐位元組比對 —— 「schema 對齊」是被驗的，不是被宣稱的。W6–W12 先長在這裡的新表另外編號、另外標明，
之後整個檔案交給後端。

`db:reset` 一個指令回到乾淨狀態，但只對它自己標記過的資料庫動手：破壞性操作之前先證明連的是可拋棄的那一份，
不是相信環境變數設對了。測試用另一個庫，跟開發庫相同就拒絕、沒設就跳過、永不退回。連線只在伺服器端。
