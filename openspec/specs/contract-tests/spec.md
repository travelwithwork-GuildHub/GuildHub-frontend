...
## Purpose
前端有兩份「後端」：真的（`guildhub`）與自己的替身（`internal`）。**唯一能防止替身漂走的東西**是這一組契約測試：
同一組測試、都走真 HTTP、對兩個目標各跑一次；golden（對真後端實錄的形狀）是兩邊共同的裁判，不是「以哪一邊為準」。

目標必須是**自己起的、可拋棄的**：`internal` 由 harness 自起 `next start`＋可拋棄 Postgres，`guildhub` 由 wrapper 自起
`./run.sh` 並拒絕借用任何已經在聽的 port；CI 只跑 `internal`（service container），`guildhub` 永遠在本機。
成對邊界（`max` 接受、`max+1` 拒絕）從 `limits.ts` 產生 —— 只送超長抓不到收緊；WS 的怪癖對替身與真後端各驗一次。
