...
## Purpose
站內信是陌生人第一次正式私訊的持久紀錄（不可 Edit／Delete），是「看見 → 靠近 → 旁聽 → 打招呼 → 正式申請」那條鏈上可選的一個落點。
後端只給一份混合的扁平清單（我寄的＋我收的，offset 分頁 20 一頁），對話是前端推導的：以對方分組、組內舊到新、組依最新新到舊、以 `id` 去重；
名字用 `GET /api/profiles/{id}` 解析、失敗顯示縮短 id 不擋。

收件匣是阻斷式面板（`PanelShell`），兩個入口：標題列「收件匣」、別人名片上的「寄信給他」（關看板、直接進對話）。
資料放 provider：分頁請求一次一個並帶世代（舊世代只合併訊息）、寄出 201 才合併並重取第 0 頁（offset 分頁會位移）、401 或身分換了整份失效；
對話裡不放「載入更多」；送出中只擋第二次送出。本地後端補 `GET／POST /api/messages`（主體條件在 SQL 的 WHERE），契約以資料驗「分頁套用在授權後的集合」。不做未讀（`BE-G06`）。
