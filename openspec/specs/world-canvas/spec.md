...
- **VERIFY-BY** `manual-browser`｜2026-09-10，於 `6f99c56` 之後的 `/world`，Chromium ＋ SwiftShader、viewport 1400×900 人工目視確認：永久地面存在；玩家與地面接觸、沒有可見的縫；**玩家在地面上留下清楚可見的陰影**（archive: fe-w10-environment-components，tasks.md 4.3）｜陰影是 GPU 算出來的畫面內容；程式能檢查的只有「有沒有設定陰影參數」，而參數設對了畫面上仍然可能沒有陰影。**截圖刻意不進版控** —— 它只證明得了「那一台機器在那個當下畫面是對的」，三個月後破圖時對除錯沒有幫助，而 `docs/DECISIONS.md` 拒絕過把視覺證據當閘門
