...
## Purpose
三種入場方式（暱稱、恢復金鑰、帳號密碼）裡只有帳號密碼在驗證身分：暱稱建的是新名片、金鑰是「拿到就是你」。
`/login` 在既有兩個表單之後多一塊「用帳號密碼」，登入／註冊在同一塊裡切換（DOM 上一次只有一個表單、切換不丟值），
匿名暱稱仍是第一個表單、仍是發表日的主路（規格書 §9：不能卡在註冊）。

限制一律來自 `LIMITS`（`login_id` 3–32、`password` 至少 8 且沒有上限、名字 1–20，code point 計），原值原樣送出；
403 與 409 是前端自己定義的領域錯誤（「帳號或密碼錯誤。」不分帳號不存在與密碼錯；「這個帳號已經有人用了。」），文案是前端寫的、
只在那一次請求上轉；成功依「記住我」處置金鑰後導向 `/world`，不看恢復金鑰畫面。本地後端補 `POST /api/register`（只認 `login_id` 的 unique 違反 → 409），
契約測試兩個目標都驗 register、密碼登入與併發撞名。
