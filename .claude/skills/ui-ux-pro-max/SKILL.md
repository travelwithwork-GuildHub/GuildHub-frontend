...
**GuildHub-frontend note (vendored copy, see `VENDOR.md`):** the skill is checked into this repo at `.claude/skills/ui-ux-pro-max/`; there is no plugin install and no `${CLAUDE_PLUGIN_ROOT}`. Run the script from the repository root with `python3` (standard library only, nothing to install):

```bash
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "<query>" --domain <domain>
```

Requires Python 3.x, no external dependencies. **Not vendored** (to stay under the repo's size cap): the `google-fonts` domain and every stack except `nextjs`, `react`, `threejs`, `html-tailwind` — a query against those returns `File not found`; that is expected, not a bug.
...
| Individual Google Fonts | `google-fonts` | *(not vendored in this repo)* |
...
**Available stacks (vendored in this repo):** `nextjs`, `react`, `threejs`, `html-tailwind`. Upstream also ships `vue`, `svelte`, `astro`, `nuxtjs`, `nuxt-ui`, `angular`, `laravel`, `swiftui`, `react-native`, `flutter`, `jetpack-compose`, `shadcn`, `javafx`, `wpf`, `winui`, `avalonia`, `uno`, `uwp` — not copied. Use the stack detected in Step 1.
