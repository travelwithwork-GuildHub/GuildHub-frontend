...
- **套件管理一律 pnpm**（2026-09-14 定案，全系統預設；理由＝同機多專案共用 store＋不放 phantom dependency）：本機與 brief 用 `pnpm install --frozen-lockfile`、`pnpm test`、`pnpm run <script>`、`pnpm exec openspec validate`，不寫 `npm ci`／`npx`。`package.json` 的 `packageManager` 欄是真源。⚠️ 過渡期：CI 仍跑 `npm ci`＋`package-lock.json`（遷移票：先模板 ai-team-starter、後本 repo），所以動相依時 `pnpm-lock.yaml` 與 `package-lock.json` **兩份都要更新**，只改一份會在 CI 或本機其中一邊裝到不同版本
