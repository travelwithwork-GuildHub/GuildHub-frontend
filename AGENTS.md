...
| 寫法 | 會怎樣 |
|---|---|
| 圍籬的開關 | 照 CommonMark：**閉合要跟開啟同字元、長度不能更短，而且不能帶語言名**。所以「用四個反引號包住一段三個反引號的示範」是安全的 |
| 表格列縮排四格以上 | **報。** Markdown 會把它當成程式碼區塊 —— 是範例就放進圍籬，是資料就把縮排拿掉（一到三格是可以的） |
| 表格列少了開頭的 `\|` | **報。** GFM 允許省略，這支腳本不支援 —— 少打一個豎線，那一列的週次、點數、阻塞、標記會整排消失，而畫面上它還是一列表格 |
