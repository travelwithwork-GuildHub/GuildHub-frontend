...
| `標記` | `Cancelled`／`Pending`／`TBD`／`Later`／`Alarm`／`Regular`，**後面接 `｜` 與理由** |
...
| **標記** | `Cancelled`／`Pending`／`TBD`／`Later`／`Regular`／`Done`／`Alarm`，後面接 `｜` 與理由 | 沒理由會紅；互斥的處置並存會紅 |
...
**〈里程碑〉是另一張表，表頭固定四欄**：`| 里程碑 | 目標週 | 驗收結果 | 靠哪些 |`。
`progress.sh` 認表頭、只讀〈里程碑〉這一節的**第一張表**（同一節底下的跨項依賴表
不會被當成里程碑）、圍籬裡的範例不算。有這一節卻一列都讀不出來、表頭認不出來、
欄數對不上、「靠哪些」重複或指向不存在的 ID、「靠哪些」寫了群組（要逐項列出工作項目 ID）、「目標週」不是 `W3`／`W3–W4`（en dash）／`—`，都會紅。里程碑 ID 的形狀不驗（它只用來顯示與查重）。舊的兩欄
`| 週 | 這一週結束時，使用者能做什麼 |` 還讀得懂，但 `--json` 會標
`coverage_status: unavailable`、`covers: null` —— **是「不知道涵蓋了什麼」，
不是「涵蓋了零項」**。它是給既有專案的過渡，三個 repo 遷完就拔。

**里程碑可追溯性（報告，不擋）**：`progress.sh` 預設就印一行（含 `--check`）——
① 幾項**已排週次**卻沒有被任何里程碑的「靠哪些」指到，② 幾項沒排週次／遠期而沒被指到；
`--trace` 列出是哪些，`--json` 的 `milestone_trace` 有完整清單。單位是 WBS ID（續行列沒有 ID），
不算 `Cancelled`／`Regular`，不用週次截斷。舊的兩欄里程碑印「未評估」——**不是**全部被指到。
它只證明有一條引用邊，不是「100% 法則」：證明不了語意上真的涵蓋，也證明不了沒有漏掉還沒寫進 WBS 的範圍。

**週次錨點（報告，不擋）**：`W1`、`決策≤W5` 本來沒有錨——W1 是哪一天沒人知道，所以決策期限
**過了之後**永遠不會出聲。在 `docs/WBS.md` 放一行（只能一行、時區寫明）：

```
<!-- wbs:week1 2026-10-05 Asia/Taipei -->
```

W1 是那一天起的七天；`決策≤W1` 在 W2 第一天才算過期。之後 `progress.sh` 每次都講今天是第幾週、
幾個缺口**過了決策期限還沒答案、fallback 已生效**（`--trace` 看是哪些）。**「有答案了」的紀錄方式**是把缺口
標 `Done｜答案…` 或 `Cancelled｜…`（缺口有 change 的話，change 全部封存也算——有 change 的項目不准標 `Done`）——不是刪阻塞邊。**它不影響 `--check` 的退出碼**：時間不能進 CI 的結果
（放一個週末，無關的 PR 會變紅；人會學會把錨點往後推）。沒設錨點但有決策期限，會提示一行「未設定」；
完全沒有決策期限的專案什麼都不講。`--as-of YYYY-MM-DD` 把「今天」換掉（測試與回放用）。

**工作項目彼此的先後寫在〈跨項依賴〉，不寫在阻塞欄**（阻塞欄只放缺口）。
`## 跨項依賴` 底下第一張表，表頭固定四欄（**要有這個標題**：從舊表遷過來時不能只原地改表頭，
四欄表頭不在這個標題底下會紅——不然那張表完全沒被讀到）：
...
| 這一項 | 依賴 | 關係 | 說明 |
|---|---|---|---|
| APP-P03 | APP-C01 | 全部 | 清單要先有骨架 |
| APP-N01 | APP-M02、APP-M07 | 任一 | 應徵或邀請都會產生 application |
```

- **這一項**恰好一個 ID。只指某一條驗收的依賴不能寫在這裡 —— 驗收沒有週次，
  排程驗不了：拆成有自己週次的項目，或移到驗收層級。同一項可以有多列，每列都要成立。
- **依賴**只能是 ID，用 `、` 分隔。**不收 `→` 與 `或`**：「至少一個」寫在關係欄的 `任一`；
  `A → B` 是兩件事，拆成「B 依賴 A」與「這一項依賴 B」兩列。
  **被依賴的項目要有工作週次**（已取消的除外），不然排不出先後：缺口（有 `決策≤Wn`）寫在阻塞欄，
  那裡的不變量 5 才看得到它；還沒排週次的先排；常態（`Regular`）沒有完成點——真的要依賴，
  拆出一個有週次的一次性項目（例如「管線可用」）再依賴它。這一張只放工作項目彼此的先後。
  **這一項自己沒有工作週次**時：缺口、常態是錯誤（缺口的決策期限是「最晚」，不是開工週，
  比不出先後——「裁決要等某個工作做完」寫在缺口的理由裡；常態拆出一次性項目）；
  **已完成卻沒有週次**是錯誤（補上實際做的週次，或刪掉這一列）；**還沒排週次**不算錯，
  `--check` 會點名，排上週次就開始驗。**這一項已取消**不比排程、會點名（有沒有週次都一樣）。
- **關係** `全部`／`任一`。**說明**是自由文字，這一欄的 ID 不算依賴（出處寫這裡）。

`--check` 會驗：排程（**被依賴的最晚週 ≤ 這一項最早週，同週合法**）、環、
「已完成（全部 change 封存，或治理項目的 `Done`）而前置還沒完成」、以及上面的文法。
舊的「已知的跨項依賴」兩欄自由文字表還讀得懂（網頁照樣顯示），但**不驗、不猜**
—— 自由文字裡的 ID 不全是依賴 —— 每次跑都會印一行「排程與環都沒有被驗」。
**遷過來時若有逆序，那是規劃要裁決的事，不是格式轉換**：改週次還是改依賴由團隊決定。

阻塞欄另外驗兩件事：缺口擋缺口繞成一圈；以及**已完成的項目還掛著指向「待裁決」缺口
的邊**（做完了，它依賴的決定卻還沒定 —— 裁決了就刪掉或改寫那條邊）。指向「等外部」
（例如 `待銜接`：本地做完、之後對齊）或「已取消」（`BE-拒` 那種：做完了但上不了線）
的邊掛在已完成的項目上是**仍然成立的資訊**，不報。
...
**沒有第三輪**。為什麼要有這條煞車、為什麼不是每個 PR 都送：`docs/DECISIONS.md`〈2026-09-19　外部 PR 審查是例外〉。
