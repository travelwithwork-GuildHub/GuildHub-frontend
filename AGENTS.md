...
FE-C01〜FE-C03     波浪號不是範圍符號（訊息會告訴你改用 –）
FE-C01/           斜線後面沒東西
FE-C1             位數不足
FE-Q01a           編號後面黏英數
...
圍籬（三個反引號或三個 `~`）與 `<!-- -->` 註解裡的內容**一律不算**，
那是格式範例。**但跳過機制自己會叫**：沒關起來的圍籬或註解會紅，
不會讓整份文件靜靜消音。

圍籬的開關**照 CommonMark 的規則** —— 閉合要跟開啟同字元、長度不能更短。
所以「用四個反引號包住一段三個反引號的示範」是安全的，
內層那些不會被誤判成關閉。
