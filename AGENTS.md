...
**適用**：`spec/` 與 `feat/` PR 的外部模型審查（codex、Gemini 各一份、平行、獨立）。
`chore/`／`archive/`／`governance/` 不跑。

**能給 REQUEST_CHANGES（BLOCK）的只有四種**，每一條都要附「規格哪一句、現在怎麼重現、最小驗法」：

1. **產品行為違反已合併的 Scenario**（或 spec PR 裡兩條 Requirement 互相矛盾、數值留到實作才決定）。
2. **判準恆真**：沒測到指定行為、拔掉核心防禦仍綠、或基本突變不會紅。
3. **一個合理的無心之過就會漏報**：正常寫法（忘了套層級、少一個 `inert`、多一層 wrapper）能讓判準綠著放過錯的東西。
4. 安全、隱私、資料損壞。

**不是 BLOCK 的**：「判準對某一種刻意規避的寫法沒有防禦」（`text-indent:-9999px`、`clip-path`、`100vmax box-shadow`、
偽元素遮罩⋯⋯）。這些是 **FOLLOW-UP**：審查者在**第一輪一次列完**，作者可以順手修或記到 PR 留言的〈量尺待強化〉清單，
**不影響 APPROVE**。第二輪起不得新增跟上一輪修改處無關的規避手法。

**輪數**：每個審查者最多 **三輪**。第一輪完整審；第二輪只驗上一輪的修改與直接回歸；第三輪裁決。
第三輪仍不 APPROVE ⇒ 不再循環，作者把爭點列成表（規格句、審查者主張、作者主張）交人類裁決。
**連續兩輪只有 FOLLOW-UP、沒有 BLOCK ⇒ 視同 APPROVE。**

**成本**：全套 e2e（500+ 條、含重 build）只在送審前與最終輪跑，中間輪只跑受影響的判準。
每輪回覆的留言開頭寫一行 `第 N 輪：產品缺陷 x、量尺強化 y、耗時 z 分`，讓輪數與收穫看得見。

**為什麼**（`docs/DECISIONS.md`〈2026-09-19　PR 雙審最多三輪〉）：#516／#517／#518 共 21 輪，真的產品缺陷 4～5 個，
其餘約 15 輪都在替 e2e 量尺加一種新的 CSS 規避防禦 —— 而假想的規避者是同一個作者自己。兩個外部模型都判「過度、不該擋合併」。
AI 審 AI 沒有時間成本的感覺，會無限找反例；上限與分類是煞車。
