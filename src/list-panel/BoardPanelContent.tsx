...
import { BOARD_VIEW_LABELS, TITLES } from './BoardPanel'
import { useListPanel } from './ListPanelProvider'
import type { ListKind } from './paging'

// 兩塊看板開出來的面板**內容**：`ListPanel` 的第一個呼叫端。規格 `FE-B01-S01`／`S02`；
// 三個插槽接的是 `FE-X04` 的 `EmptyState`（`FE-X04-S11`）。
//
// ⚠️ **這個檔案被 `PanelHost` lazy 載入**（`FE-X15` --panel-board；design D3）：它是看板的重模組
// （`CreateProjectForm`／`ProjectDetail`／`MyProjects`／`TalentDetail`／`OwnerActions`⋯⋯），開啟意圖成立前不進首屏 chunk。
// **世界輸入鎖不在這裡** —— 面板 lazy 後鎖要在 chunk 抵達前就成立，只有 eager 的 `ListPanelProvider` 做得到（綁開啟意圖、不綁這裡的殼掛載）。
//
// ⚠️ **翻譯在這裡，不在容器裡。** 容器把原始的失敗交出來，這裡 `toUiError`，
// `EmptyState` 再依 `kind` 決定畫成載入失敗還是權限阻擋（`FE-X04` design `D3`）。
// 訪客按 E 拿到 401 → 「要先登入才看得到這裡。」——不再是一片空白。
//
// 權限阻擋的動作刻意沒給：require-name 之後訪客根本進不了世界（先過取名門檻），
// 這條 401 主要落在 `unavailable`（後端問不到身分、被放行進世界）身上；那時標題列仍有「建立你的身分」的安全閥入口。
// 登入／登出還沒做（`FE-A08`／`FE-A02`），要不要、去哪裡是之後的事。
//
...
// `PanelHost` 的 `load` 拿到的 default：讀「開著哪一塊」、渲染對應的看板。host 只在 `open !== null` 時開這個面板，
// 但萬一 chunk 到達時已經關掉（過期），這裡再擋一層（`open === null` 回 null）。
// 分兩支寫而不是一個 `renderItem: (item: A | B)`：型別讓「案件面板拿到人才資料」在 typecheck 就紅。
export default function BoardPanelContent() {
