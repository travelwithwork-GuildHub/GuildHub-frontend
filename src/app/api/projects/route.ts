...
//
// body 用 `contract.ProjectCreate` 解析 —— 它只有型別與預設值（`needed_skills` `[]`、`seat_count` 整數、預設 4），
// 跟真後端的 Pydantic `ProjectCreate` 對得上（`str`／`str`／`list[str] = []`／`int = 4`）。**這裡不擋長度與範圍**：使用者面向的上限由前端的 `FORM_LIMITS` 守，
