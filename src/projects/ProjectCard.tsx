...
const CHIP = withClass(CAPTION, 'bg-surface whitespace-nowrap rounded px-1.5 py-0.5')
...
export function ProjectCard({ project, now, onOpen }: { project: ProjectOut; now: number; onOpen: (id: string) => void }) {
  ...
  return (
    ...
    >
      <span data-testid="project-card-title" {...withClass(HEADING, 'text-ink line-clamp-2 block')}>
        {project.title}
      </span>
      <span {...withClass(CAPTION, 'text-ink-muted flex flex-wrap items-center gap-x-3 gap-y-1')}>
      ...
      {project.needed_skills.length > 0 ? (
        <span className="flex flex-wrap gap-1">
          {project.needed_skills.map((skill) => (
            <span key={skill} data-testid="project-skill" {...CHIP}>
              {skill}
            </span>
          ))}
        </span>
