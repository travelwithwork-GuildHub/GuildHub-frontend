...
// ⚠️ **新增與修改的行為都在 `ResourceForm`**（同一個元件：`resource` 給了就是修改）；這裡只管它開在哪一筆上。
//    **刪除**今天仍然只是「在不在」（`S03`／`S10` 驗的正是這件事），按下去不做事 —— 行為與確認層在 `--delete`，
//    上限（`S16`）也在那一支。
...
function Row({ resource, writer, onEdit }: { resource: ProjectResourceOut; writer: boolean; onEdit: (resource: ProjectResourceOut, from: HTMLButtonElement) => void }) {
  return (
...
export default function OpenResourcesPanel() {
  ...
  useEffect(() => {
    if (!formOpen && wasOpen.current) opener.current?.focus()
    ...
    >
      ...
      <div ref={root} tabIndex={-1} data-testid="resources-panel-root" data-phase={state.phase} className="flex min-h-0 flex-1 flex-col gap-gutter overflow-y-auto outline-none">
        ...
        {items !== null && items.length > 0 && (
          <ul className="flex flex-col gap-gutter">{items.map((resource) => <Row key={resource.id} resource={resource} writer={writer} onEdit={openEdit} />)}</ul>
        )}
        {/* ⚠️ 上限（`S16`：滿了就不能新增、而且說得出為什麼）在 `--delete` —— 它的另一半「刪一筆就恢復」要刪除。 */}
        {writer && (
          <button
            ref={createButton}
            type="button"
