...
//
// **晚到的回應**：讀取與確認各有自己的序號，只有「比上一次套用的更新」才套用 —— 尤其一個晚到的
// `active` MUST NOT 把已經呈現的「已結案」改回可寫入的樣子（`S36`）。
...
export function createResourcesStore() {
  ...
  const entryOf = (projectId: string): Entry => {
    ...
    const created: Entry = { state: LOADING, items: [], readSent: 0, readApplied: 0, confirmSent: 0, confirmApplied: 0, reads: 0, opens: 0 }
  ...
  /** D2 的那一次確認。`original` 是原本那個 403／409 —— 確認不是 `closed` 時畫面照它呈現。 */
  const confirmStatus = (projectId: string, entry: Entry, original: UiError) => {
    const mine = (entry.confirmSent += 1)
    void getProject(projectId).then(
      (project) => {
        if (mine < entry.confirmApplied) return
        entry.confirmApplied = mine
  ...
  const read = (projectId: string) => {
    ...
    entry.reads += 1
    const mine = (entry.readSent += 1)
    set(entry, LOADING)
    void listResources(projectId).then(
      (items) => {
        if (mine < entry.readApplied) return
        entry.readApplied = mine
        entry.items = items
      ...
      (cause: unknown) => {
        ...
        entry.readApplied = mine
        const ui = toUiError(cause)
        if (ui.kind === 'permission-denied' || ui.kind === 'conflict') confirmStatus(projectId, entry, ui)
