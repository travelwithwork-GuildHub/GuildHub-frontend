...
| FE-A05 | Avatar | 角色選擇流程與即時預覽。**MVP 先做預設角色 N 選 1**。**另外接手 `FE-A06` 的「角色選擇」那一段**。⚠️⚠️ **它自己有一個前置條件：世界要先畫得出不同的外觀**（見下一列）—— 在那之前做出來的選擇器，使用者選完之後自己的外觀不變、別人看到的他也不變，那是製造錯誤期待不是 MVP | W2 | 8 | | TBD｜等 `FE-W19` 的最小可見閉環，否則選擇器是假的 |
