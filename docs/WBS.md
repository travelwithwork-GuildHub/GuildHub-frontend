...
| **項目** | 模組名稱 |
| **工作** | 一條可以認領的工作 |
| **週** | **目前核准的相對執行順序，不是日期承諾。** 同一週內以表格順序或明示的依賴決定先後。寫 `—` 代表**沒有排程**（等外部或待裁決），它不計入「未開始」。**2026-09-17 起到 demo 為止，執行順序以〈demo 之前的核准順序〉那一節為準**，那一節列到的項目週次已對齊；沒列到的排在整份順序之後。缺口這一欄放的是 `決策≤Wn`，那是**決策期限，不是工期** |
| **點** | 估點 |
| **阻塞** | 這一列要跟外部對齊什麼：`待銜接`（本地先做了，之後要對齊）／`後端行為有誤`／`BE-拒`（後端明文不做，做了上不了線）／`待裁決`／`外部`，前面接缺口的 ID。**填了這一欄不代表做不了** —— 缺口清單在 `BE-G` |
| **標記** | 人為決定，見下 |

### 這張表有文法

`progress.sh --check` 是一個解析器。**四個欄位各自只能寫哪些東西**、
ID 引用有哪幾種合法寫法、範例為什麼一定要放進圍籬 ——
完整規則在 `AGENTS.md`〈改 `docs/WBS.md` 之前〉。最常踩到的三個：

- **週次的範圍用 `–`（en dash），不是 `-`。** 打成 ASCII 連字號的話，
  那一項的排程會靜靜消失、翻成「沒有排程」
- **阻塞欄的類型要先在下面〈阻塞類型〉那張表裡宣告過。**
  那張表就是詞彙的來源，腳本裡沒有寫死任何一個詞
- **敘述裡要放豎線就寫反斜線加豎線**，不然欄位會整排右移
...
| FE-A05 | 已封存 | `fe-a05-avatar`、`fe-a05-avatar-variety` |
| FE-A08 | 已封存 | `fe-a08-account-login` |
| FE-B01 | 已封存 | `fe-b01-list-container` |
| FE-B02 | 已封存 | `fe-b02-project-card` |
| FE-B03 | 已封存 | `fe-b03-project-detail` |
...
| FE-O08 | 已封存 | `fe-o08-guildhub-rehearsal` |
| FE-O09 | 已封存 | `fe-o09-env` |
| FE-O11 | 已封存 | `fe-o11-coverage`、`fe-o11-evidence` |
| FE-O14 | 已封存 | `fe-o14-drop-preview-notice`、`fe-o14-preview-deploy`、`fe-o14-rest-build-gate` |
| FE-O20 | 已封存 | `fe-o20-path-params` |
| FE-O21 | 已封存 | `fe-o21-boundary-lint` |
| FE-R01 | 已封存 | `fe-r01-realtime` |
| FE-R02 | 已封存 | `fe-r02-protocol` |
| FE-R03 | 已封存 | `fe-r03-position-sync` |
| FE-R05 | 已封存 | `fe-r05-self-echo` |
| FE-R06 | 已封存 | `fe-r06-multi-tab` |
...
| `demo 之後` | **決定要做，但排在 demo 之後。** 不算「未開始」，預設不列出 | 標記 `Later` |
| `常態` | 沒有完成點的持續性工作 | 標記 `Regular` |
| `已完成` | 做完了，但**對不到任何 change**（治理工作） | 標記 `Done`，理由欄要寫出憑什麼 |
| `矛盾` | 標了不做（或 demo 之後）卻有 change 已封存；或標了 `Done` 卻有 change | 兩份紀錄打架，**腳本不挑一邊信** |

前五個是**事實**（git 與 OpenSpec 證明得了），後六個來自**人寫的標記**。
...
| `Later` | 決定要做，但**排在 demo 之後**（2026-09-19 起：demo 只做「後端已有的 API 都有對應操作」那一圈） | demo 結束、重排週次之後 |
...
## demo 之前的核准順序（2026-09-17）

> **主軸：後端已經提供的每一個端點與 WS 訊息，前端都要有對應的、使用者做得到的操作。**
> 這是使用者 2026-09-17 三輪修正後定下的（第一輪只到「觀看」、第二輪漏掉整組案件 API，
> 兩輪都被打回）。原話：「你要把這些 api 實作的功能都完成」。
>
> 三條規則，改動任何排程之前先讀：
>
> 1. **要靠後端沒有的 API 才動得了的功能，demo 之前不開。** 不是砍需求，是延後 ——
>    那些列留在各組（週次、阻塞不動），標記 `Later`，並在下面〈demo 之後〉列成一份，
>    免得有人再排進來。「後端沒有」的判準是 `src/api/contract/schema.d.ts` 沒有那個操作。
>    ⚠️ **2026-09-19 改**：原本「標記都不動」，於是 `progress.sh` 把 70 幾項算進「未開始」，
>    看的人分不出「demo 前還要做」跟「之後才輪到」（使用者原話：「我們現在開發都是以 demo 為主，
>    把 api 串完、完成前端功能，其他就之後再做，不要搞混」）。現在 **demo 範圍外的一律標 `Later`**，
>    不只是要靠後端的那些 —— 狀態叫「demo 之後」，不算未開始。
> 2. **純前端做得到、而且在下表順序裡的照做**（視覺、載入、名字牌、狀態 UI、斷線復原）——
>    「後端沒有」不是它們的理由。不在下表的純前端項目一樣標 `Later`。
> 3. **沒有後端的入口不上畫面，連灰掉的按鈕都不放**（使用者原話：「目前後端那邊沒有規劃，
>    可以先隱藏，不然也沒辦法使用」）。2026-09-17 盤過 `src/`：今天畫面上沒有這種入口，
>    之後也不准出現。

後端 `c6f3928` 的端點對前端功能的覆蓋（**未接的那六個 REST 操作就是這一輪的主體**）：

| 端點／訊息 | 前端今天 | 去哪一項補 |
|---|---|---|
| `POST /api/login` 三種模式、`POST /api/register`、`GET /api/me` | 有 | 三種登入去向不一致 → `FE-A06` 補一刀 |
| `GET /api/profiles`、`GET /api/profiles/{id}`、`PATCH /api/profiles/me` | 有 | — |
| `GET /api/projects` | 半：看板只列標題一行 | `FE-B02` |
| `GET /api/projects/{id}` | 無 | `FE-B03` |
| `POST /api/projects` | 無 | `FE-J01` |
| `POST /api/projects/{id}/form-team`、`POST /api/projects/{id}/close` | 無 | `FE-J04` |
| owner 找自己的案子（前端用 `owner_id` 過濾） | 無 | `FE-J03` |
| `GET /api/rooms`、`POST /api/projects/{id}/enter` | 有 | — |
| `GET /api/projects/{id}/seats`、`POST /api/projects/{id}/seats` | 無 | `FE-J13` |
| `POST /api/messages`、`GET /api/messages` | 有：收件匣、人才頁私訊 | 案件頁私訊、對話串 → `FE-K01` 補一列 |
| WS `move`／`pos`／`snapshot`／`chat` | 有 | — |
| WS `presence`（`name`、`av`） | 半：外觀畫了、名字沒畫 | `FE-W08` 的名字那一列 |
| WS `status` | 無：收得到、沒 UI 送也沒畫 | `FE-K05` |
| 斷線重連（後端每次重部署 WS 全斷） | 無 | `FE-R12` |

**順序**（做完 #3～#10 就能在閘道上從頭演：取名 → 發案 → 看板 → 詳情、私訊發案者 →
成軍設密碼 → 門 → 進房坐位 → 結案）：

| 順序 | 工作項目 | 使用者能做什麼 | 點 |
|---|---|---|---|
| 0 | 這份 governance ＋ `archive/fe-o14-rest-build-gate` | — | 2 |
| 1 | `FE-O08` 演練 | 用真後端量 form-team／close／seats 的實際狀態碼與 409 內容 —— **#3～#8 的規格靠這個** | 4 |
| 2 | `FE-A06` 入口一致 | 三種登入都直接進世界 | 3 |
| 3 | `FE-J01` 建案 | 看板旁「發案」→ 送出立刻上看板 | 6 |
| 4 | `FE-B02` 案件卡片 | 標題、需要技能、狀態、剩幾天、座位數 | 5 |
| 5 | `FE-B03` 案件詳情 | 描述、發案者名片、「私訊發案者」；owner 看到成軍／結案鈕 | 8 |
| 6 | `FE-J04` 成軍／結案 | 設密碼成軍 → 門出現、密碼可複製與一鍵私訊；結案門消失 | 5 |
| 7 | `FE-X16` DOM 視覺與動線 | 畫面看起來有設計、一次只開一個面板、看得出下一步按哪裡 | 8 |
| 8 | `FE-J03` 我的案件 | 我發的案子、各自狀態、直達成軍／結案 | 4 |
| 9 | `FE-J13` 座位 | 進房看誰坐哪、一鍵入座、被搶有提示 | 7 |
| 10 | `FE-K01` 補片 | 案件頁私訊、對話串、回覆 —— **已由 `FE-B03`（#503「私訊發案者」）與 `FE-K01` 本體（對話串、回覆）涵蓋，2026-09-20 確認不另開 change** | 3 |
| 11 | `FE-K05` 狀態 | 快捷狀態＋自由輸入，別人看得到 | 5 |
| 12 | `FE-W08` 名字牌 | 附近的人頭上有名字（只畫附近／鏡頭內） | 3 |
| 13 | `FE-R12` 斷線復原 | 後端重部署不用重整、沒鬼影 | 13 |
| 14 | `FE-X15` 首屏載入 ＋ `FE-O12` | 打開就有畫面、按鍵有反應；Rapier 與面板延後載 | 12 |
| 15 | `FE-W20` 看板摘要 | 不按 E 也看得出看板有沒有案子 | 8 |
| 16 | `FE-O15` 發表準備 | 固定腳本、reset 策略、rehearsal | 17 |
| 17 | `FE-X09`、`FE-W14` | 弱裝置、好看 | 15 |

每一項都走 spec → feat → archive，看得見的功能要有真瀏覽器 e2e，每個 PR 回報一句效能影響
（基線：`/world` HTML 293 ms、canvas 1315 ms、18 個資源 998 KB、最大 chunk 711 KB 是 Rapier）。

**demo 之後**（要靠後端沒有的 API；對應的缺口在括號裡）：
`FE-M01`–`FE-M10`、`FE-N01`–`FE-N07`（`BE-G10` 應徵／邀請／Offer 沒有模型）；
`FE-J02`、`FE-J05`–`FE-J12`（`BE-G10`、`BE-G22` 生命週期只有三態）；
`FE-J14`（`BE-G12`，別人負責）；`FE-B05`–`FE-B07`（`BE-G05`、`BE-G23`）；
`FE-K02`、`FE-K03`（`BE-G11`）；`FE-K01` 的已讀那一列（`BE-G06`）；
`FE-T01`–`FE-T05`（`BE-G24`、`BE-G15`）；`FE-A03`、`FE-A07`（`BE-G26`）；
`FE-V02`–`FE-V06`、`FE-V08`–`FE-V14`（`BE-G27`、`BE-G09`、`BE-G13`、`BE-G14`）；
`FE-B10` 的回訪入口（`BE-G01`、`BE-G23`）。
其餘沒列在上表、也不在這一段的項目（例如 `FE-W17`、`FE-W18`、`FE-V07`、`FE-X10`、
`FE-A02` 登出、`FE-X02` 狀態管理）是純前端可做，但**不在 demo 範圍**，2026-09-19 起一樣標 `Later`；
不是不做，是 demo 之後。要拉回 demo 就把 `Later` 拿掉並排進上表。

**送回後端的清單**（`FE-O08` 之後一併送）：`BE-G10` 什麼時候有（第一條）；`BE-G27` 活動物件；
Demo reset；重部署時的 close code；room token TTL；`/api/rooms` 只回 active 與 seed 房間穩定性；
nickname 超過 20 字回 500 不是 422；座位 409 要分「被搶」與「已坐別格」；demo 期間不重部署。
...
|---|---|---|---|---|---|---|
| BE-G01 | ~~**身分無法跨裝置恢復**~~ **後端已修（2026-09-07，`f278e2c`）** | `POST /api/login` 現在收 `resume_token`：帶著它是拿回既有的名片，不建新的（`app/api/auth.py:60`，`tests/test_resume_token.py`）。⚠️ **前端還沒用它** —— `src/api/contract/rest.ts` 的 `LoginIn` 要補這個欄位，而且要決定 token 存哪裡。〔以下是原本的缺口描述〕`POST /api/login` 每次都 `uuid4()` 新建一張名片（`app/api/auth.py:14`）—— 同一個暱稱登入兩次是兩個人；身分只活在 session cookie，清 cookie 或換裝置就永久失去專案與訊息。`CLAUDE.md` 又明文排除 OAuth／第三方登入、帳號刪除、資料匯出。**W12 的回訪／最近專案／未讀全部不成立**，前端不要承諾 —— **最晚 W2 開工前要答案。** 沒答案就照 (c) 走：接受「每次都是新的人」，把回訪相關項目全部標 `Cancelled` 並寫明原因。 **【沒答案就】**接受「每次都是新的人」，把回訪相關項目全部標 `Cancelled` 並寫明原因。 | 決策≤W1 | — | `BE-拒` | Done｜**已接上並實測**（2026-09-10）。`LoginIn` 補了 `resume_token`（PR #240），`src/identity/` 用它做恢復（PR #242），端到端驗過「清掉 cookie 之後勾了記住我的人回得去」與「換一個全新環境只貼金鑰取回同一張名片」（PR #245，`tests/e2e/identity-flow.mjs`） |
| BE-G02 | ~~**世界裡每個人的名字都是「訪客」**~~ **後端已修（2026-09-06，`bfb3609`）** | `auth.py:27` 現在寫 `session["name"] = row["display_name"]`，`main.py:101` 讀得到。⚠️⚠️ **但前端看不到差別，因為前端從來不登入** —— `/world` 是匿名連線，`_identify` 走匿名路徑，名字仍然是「訪客」。**要看到名字，`FE-A02` 登入必須先做。**〔以下是原本的缺口描述〕`app/main.py:79` 回 `session.get("name") or "訪客"`，但 `session["name"]` 在整個後端**從來沒有被設定過**（`auth.py:27` 只設 `user_id`）。FE-W09「顯示 Display Name」做出來會全部一樣，**看起來像前端壞了** —— **最晚 W3 開工前要答案。** 沒答案的 fallback：世界裡不顯示名字（只顯示狀態），**不要顯示一堆「訪客」**。 **【沒答案就】**世界裡不顯示名字，只顯示狀態 —— **不要顯示一整片「訪客」**。 | 決策≤W1 | — |  | Done｜**已接上並實測**（2026-09-10）。卡的是 `FE-A01` 不是 `FE-A02`（登出是 `FE-A02`）。登入之後從瀏覽器開的 WS，第一則 snapshot 是 `{"name":"名字測試員","av":0,…}` —— 不再是「訪客」（PR #245）。⚠️ **世界目前還沒有把名字畫出來**，那是 `FE-W08`／`FE-R10` |
| BE-G03 | ~~**遠端玩家一律 avatar 0**~~ **後端已修（2026-09-07，`fd8c00f`）** | `manager.py:98` 現在把 `avatar_id` 傳給 `presence.join()`，而它來自 `session["avatar_id"]`（`auth.py:32`）。⚠️ **同 `BE-G02`：前端不登入就仍然是 0。**〔以下是原本的缺口描述〕`manager.py:91` 呼叫 `presence.join()` 時沒傳 avatar，`presence.py:45` 預設 `0`。就算前端把外觀做完，別人看到的還是同一隻 **【沒答案就】**固定預設角色，**不做角色選擇 UI** —— 做了別人也看不到。 | 決策≤W1 | — |  | Alarm｜後端已修，登入之後 `avatar_id` 也送得到了（實測 `av:0` 是因為還沒有選角色 UI）。**卡的是 `FE-A05` Avatar**，不是登入 |
| BE-G04 | **`avatar_id` 的語意是「角色圖索引」，不是六維編碼欄位** | `API-前端整合指南.md:185`「smallint，前端據此挑角色圖」。把它當成 6 維 × 各 4 種的位元編碼，是**重新詮釋一個已經有語意的欄位**。MVP 先做**預設角色 N 選 1** —— **最晚 W2 開工前要答案。** 沒答案就做預設角色 N 選 1，並且**不宣稱多人可見的角色選擇已完成**。 **【沒答案就】**做預設角色 N 選 1，並且**不宣稱多人可見的角色選擇已完成**。 | 決策≤W1 | — |  | Done｜**裁決＝`avatar_id` 就是角色圖索引**（後端文件本來的語意，不重新詮釋成編碼欄位）。前端已照此落地：`FE-W19` 的 `av` → 外觀對照（`src/design/avatar.ts`），`FE-A05` 做成 8 款索引＋首次建立身分隨機發一款（#526／#527／#528，2026-09-19）。**六維外觀沒有被這個裁決做掉** —— 真要做，得另開欄位，不能塞進 `avatar_id`；它留在 `FE-W19` 底下那一列、標 `Later` |
...
| BE-G32 | **`PATCH /api/profiles/me` 不會更新那條連線背後的 session** | `avatar_id` 是 `auth.py` 的 `_remember()` 在**登入時**寫進 session 的，而 `presence.join()` 從那裡讀。所以使用者改了 profile 之後，**已經在場的其他人看到的仍然是舊外觀** —— 那條連線背後的 session 沒有跟著改。⚠️ **前端已經有解**（`FE-A05`：儲存成功之後主動重新建立連線），所以這一條**不擋任何前端工作**；但它有一個使用者看得到的代價，而那個代價已經寫進規格 `avatar-selection` 的 `S05`：**重連期間其他人可能短暫看到這個人離開又進來**。後端若能在 `PATCH` 成功時同步更新 session（或推一則 presence 更新），就能把那個現象整個消掉。**【沒答案就】**維持前端現在的做法（儲存成功後主動重連），並接受規格 `S05` 已經寫明的那個代價 —— 也就是**這一條沒有答案不會擋住任何東西**。 | 決策≤W5 | — | | Pending｜**不擋前端**（`FE-A05` 已經自己解決了），但它是 `S05` 那個「離開又進來」唯一的根治方式 |
| BE-G31 | **`presence.join()` 對既有的 `user_id` 會重建 `Player`，把位置歸零** | `presence.py` 的 `join()` 是 `self._players[user_id] = Player(...)` —— 用一個座標歸零的新 `Player` 覆蓋掉舊的。實測（同一個瀏覽器登入後開兩個分頁）：`分頁1 走到 x=320 → 分頁2 連上 → 分頁1 被瞬移回原點`，而且分頁1 會收到一則裡面是它自己的 `presence.join`。⚠️ **前端已經做了止血**（`FE-R06` 的 `BroadcastChannel` 協調，2026-09-10），但它擋不住三種情況：分頁崩潰、同一瞬間的競態、**以及跨裝置或跨瀏覽器用同一個帳號登入 —— 最後那一種前端完全沒有辦法**，因為協調只在同源的同一個瀏覽器裡有效。根治要兩件事：`join()` 對既有的 `user_id` 不得重建 `Player`；**把連線數與玩家狀態分開**（`disconnect` 已經有 `still_here` 檢查，`join` 沒有對應的那一半）。**【沒答案就】**維持前端的止血，並接受跨裝置同帳號會互相覆寫位置（`FE-R06` 的規格已經明文寫下這一點） | 決策≤W5 | — | | Alarm｜**前端的止血擋不住跨裝置**，而那條路徑今天就存在 |
| BE-G33 | **`disconnect()` 只查同一個 scene 的兄弟連線 —— 同一人換 scene 時，新連線可能被舊連線的斷線清掉** | `manager.py` 的 `disconnect()`：`still_here = any(other.user_id == conn.user_id for other in self.scenes.members(conn.scene))`，不在**同一個 scene** 就 `presence.clear(user_id)`。而 `presence._players` 是**單一** `dict[user_id, Player]`（`BE-G31` 的同一個根）。所以同一個人從 `lobby` 換到 `room:x`：新連線先 `join()`、舊連線的 `disconnect()` 後到 → 被清掉的是**新的那個人** —— 連線開著，房間裡沒人看得到他，他的 `move` 也不記（`set_position` 對不存在的人回傳）。⚠️ **前端已經做了降機率**（`FE-V01`，2026-09-14）：換 scene 時新連線等舊 socket 的 close 事件（上限 1 秒）才建 —— close 事件代表伺服器傳輸層回了 close 幀，那之後 `disconnect()` 幾乎必然排在 `join()` 之前；**但那不是證明**，應用層跑完沒有客戶端看不到。同一個帳號在兩個分頁各連一個 scene 也是同一個洞（前端的分頁資格已改成不分 scene，見 `FE-V01` 規格的〈一個身分一條世界連線〉）。根治：`disconnect()` 查**所有** scene 的同 `user_id` 連線、或 presence 以連線為鍵。**【沒答案就】**維持前端的等待，並接受極少數情況下進房間的人是「看不見的」（重新整理會好） | 決策≤W5 | — | | Alarm｜**跟 `BE-G31` 同根**；前端的等待只是降機率 |
...
| FE-O07 | 銜接清單 | **自動產生**：本地後端實作了、真後端沒有的操作。每一項附「我們怎麼用它」與「本地版的行為」 | W3 | 9 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
...
| FE-O12 | 效能預算 | FPS、Draw Calls、Memory、WebSocket 流量、bundle size、首次載入、3D 初始化時間的**數字目標**，超過就紅。（40 人渲染的量測在 `FE-W13`；首屏的載入順序在 `FE-X15`） | W7 | 7 | | |
| FE-O13 | 視覺回歸 | 3D 畫面怎麼測 —— 截圖比對還是只測 DOM。**先決定，不要做一半** | W5 | 4 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
...
| FE-O15 | 發表準備 | 固定 demo 腳本（照〈demo 之前的核准順序〉那條閉環）、World 預載、異常 fallback、完整 E2E rehearsal。**Demo reset 在真後端做不到**（沒有端點）—— 策略改成固定 `resume_token` 的名片 ＋ demo 前一天新建的案件 | W8 | 7 | | |
| | | **Demo 資料與正式資料的隔離** —— reset 會不會動到真的東西 | W8 | 2 | | Alarm｜發表當天最不想踩的地雷 |
| | | Local fake player（spawn / wander / idle at board、status rotation）。**不進 DB、不送 WebSocket** | W8 | 5 | | |
| | | 走完一次完整流程並留下 evidence（不是只說「已完成」） | W8 | 3 | | |
| FE-O19 | 契約哨兵的新鮮度 | `src/api/contract/schema.d.ts` 是**人工**重產的，`GENERATED.md` 檔頭就寫著「這個哨兵會過期，而且不會有人告訴你」。⚠️ **2026-09-10 證實了那句話**：後端從 `27c3077` 走到 `cd2929c`（六個 commit、兩天），期間 `drift.ts` 一路綠燈；重產之後**立刻紅了兩處**（`_coverage` 少了 `RegisterIn`、`_LoginIn` 的 `nickname` 從必填變選填）。要做的是「CI 從一份**釘住版本的**後端 OpenAPI 重產，並要求工作樹零差異」——不是叫 CI 連活的後端（`AGENTS.md`：CI 不提供任何服務）。判準：改掉後端的任一 endpoint／method／entity 而不更新產出物，CI 必須紅 | W3 | 5 | | Alarm＋Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排；**它綠得合乎設計，所以沒有人會發現它過期** |
...
| FE-O16 | 技術可觀測性 | 前端錯誤回報、WS 斷線率、FPS 遙測。**只做技術 telemetry** —— 使用者行為追蹤被後端明文永久排除（BE-G17），兩者不要混在同一個提案裡 | W13–W16 | 6 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-O17 | 規模化 | `PAGE_SIZE=20` 的 offset 翻頁在資料變多時會慢且會漏；快取與預取策略 | W17–W20 | 6 | BE-G05 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G05）；等真後端提供更好的查詢；本地端先做好快取與預取 |
...
| FE-A02 | 登出 | 終止目前 session、多分頁怎麼反映、**WS 是否立即斷線**、清掉 room token 與本地敏感快取、登出後回到哪裡 | W2 | 5 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| | | **匿名身分登出後能不能再登入回來** —— 目前不能（BE-G01），UI 要講清楚，不要讓人以為登出是安全的 | W2 | 3 | | Alarm｜後端已修（`resume_token`），但**前端的 `LoginIn` 還沒有那個欄位** |
| FE-A03 | 持續身分 | 唯一帳號、名稱歷史、帳號年齡。**匿名不是問題，可被任意冒充才是** | W10 | 6 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-A04 | Profile | 顯示／編輯：Display Name、Skills、Hours、Bio；RHF + Zod + 更新 | W2 | 8 | | |
| FE-A05 | Avatar | 角色選擇流程與即時預覽。**MVP 先做預設角色 N 選 1**。⚠️ **這一列做完了，而它沒有接手 `FE-A06` 的「角色選擇」那一段** —— 首次進入流程在 `/`，**那一頁沒有 3D Canvas**（使用者是按下「進入世界」之後才第一次看到畫布），而規格談定的即時預覽是「直接改世界裡的自己」，在那裡做不到。兩個外部審查者獨立選了「選擇器只放在 `/world`」，並都主動要求把這件事寫在這裡，**不要假裝原範圍完成了**。另外兩條路都更貴：在 `/` 放第二個 WebGL context，或者做一套靜態縮圖（**而縮圖會跟實際渲染漂，那是兩套預覽**）。⚠️⚠️ **它自己有一個前置條件：世界要先畫得出不同的外觀**（見下一列）—— 在那之前做出來的選擇器，使用者選完之後自己的外觀不變、別人看到的他也不變，那是製造錯誤期待不是 MVP | W2 | 8 | | TBD｜等 `FE-W19` 的最小可見閉環，否則選擇器是假的 |
| FE-W19 | 角色外觀吃得到 `av` | **最小可見閉環**：讓 `ChibiPlayer` 依協定的 `av` 值呈現至少 N 種**可辨識**的差異（本地與遠端都要）。⚠️ **不必等完整的 `FE-W08 ProceduralAvatar`**（10 點）—— 那是 codex 在 `FE-A06` 審查時指出的：`FE-A05` 卡住的不是 `avatar_id` 的語意裁決，是「選擇根本沒有可觀察結果」。⚠️ 後端對**遠端**玩家的 `av` 目前實測仍是 0（`BE-G03` 卡在沒有選角色 UI），所以在 `FE-A05` 之前**不能宣稱「其他人會看到你的選擇」** | W3 | 3 | | Alarm｜**`FE-A05` 的前置條件**。沒有它，任何角色選擇器都是假的 |
| | | 六維外觀（Body / Hair / HairColor / Outfit / OutfitColor / Skin）與編碼 | — | — |  | Later｜demo 之後（2026-09-25）：`BE-G03` 已修、`BE-G04` 已裁決為「`avatar_id` 是角色圖索引」—— 兩條阻塞都解了，剩下的是「要不要做」的產品題；要做就得另開欄位，不能塞進 `avatar_id` |
| FE-A06 | 首次進入 | **取名字 → 帶走恢復金鑰 → 進入 Guild Hall**；已建立過的直接恢復。⚠️ **「角色選擇」從這一列移到 `FE-A05`**，而推翻它的是一個事實：`ChibiPlayer` 的顏色是模組層級常數，完全沒有 avatar 參數，`av` 雖然協定送得到也進了 `RemoteIdentity`，**沒有任何地方讀它來渲染** —— 世界今天畫不出不同的外觀。Gemini 逐字：「前端給使用者選了顏色，進去卻是一坨預設的灰塊⋯⋯這不叫 MVP，這叫欺騙」 | W2 | 6 | | Alarm｜**發表前阻擋項目。** `FE-A01` 的 design D5 把「`/world` 不擋匿名」這個中間解建立在一個條件上，而條件就是這一列：兩個審查者第二輪都同意「`FE-A01` 只做登入本身 ＋ 顯示身分、導流留給 `FE-A06`」，codex 的但書逐字是「唯一條件是 `FE-A06` 必須列為發表前阻擋項目；若發表版本可能只交付 A01、不交付 A06，Gemini 的擔憂仍成立」。**不交付這一項，發表日「每個人有名字」就破局** —— 訪客可以整場不登入。**另外承接一條發表日風險**：封存前審查時兩個審查者一致指出「陌生人不會保存恢復金鑰，離開之後永久拿不回剛建立的名片」——(a) 放在登入畫面太早（玩家還沒體驗到價值就逼他存金鑰）、(c) 帳號密碼太重，所以落在這一列。**判準**：使用者離開首次進入流程之前，畫面 SHALL 說明「恢復金鑰是再次使用這張名片的唯一方式，任何取得它的人都能使用」，並提供可操作的複製功能與複製成功的回饋；使用者 SHALL 能在一個全新的瀏覽器儲存空間裡只憑那把金鑰取回同一張名片。**突變條件**：拿掉提示或複製入口，這一項的驗收要變紅 |
...
| FE-A07 | 聯絡偏好 | 誰可以私訊我、是否接受陌生邀請、狀態對誰可見、離線時是否接收通知 | W11 | 6 | BE-G26 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G26） |
...
| ID | 項目 | 工作 | 週 | 點 | 阻塞 | 標記 |
|---|---|---|---|---|---|---|
| FE-B01 | 共用清單容器 | Panel Layout、列表、**`page` 翻頁**（0-based、`PAGE_SIZE=20`，沒有 total／`has_more`／`limit` —— 端點收的是 `page`，換算成 SQL offset 是後端內部的事；**沒有 `limit` 代表不能多抓一筆探測有沒有下一頁**）、Loading／Error／Escape。**案件與人才共用這一個**，不要各做一份 | W2 | 8 | | |
| FE-B02 | 案件列表 | 卡片要能一眼判斷：標題、需要技能（`needed_skills`）、狀態（招募中／已成軍／已結案）、剩幾天到期（`expires_at`，建立後 7 天）、座位數。⚠️ **2026-09-17 縮成後端有的欄位**：沒有 Open Role、預算、期程、應徵數（`BE-G10`、`BE-G20`、`BE-G21`），卡片上不放 | W5 | 5 | | |
| | | 排序：最新／即將到期／符合度 | — | — | BE-G05 待銜接 | Pending｜後端只有 `page`，排序要等後端；demo 之前不做 |
| FE-B03 | 案件詳情 | `GET /api/projects/{id}`：描述、需要技能、狀態、到期、座位數、發案者名片（`GET /api/profiles/{owner_id}`）、**「私訊發案者」**（`POST /api/messages`）；owner 看到「這是你發的案子」的標示與一個動作插槽（成軍／結案的**可見入口與動作**都在 `FE-J04`，接進這個插槽 —— 沒有 handler 的按鈕不先放）。⚠️ **2026-09-17 縮成後端有的欄位**：沒有 Open Roles、期程、應徵狀況（`BE-G10`、`BE-G21`） | W5 | 8 | | |
| | | 發案者資訊：帳號年齡、發過幾個案子、回覆率、平均回覆時間、目前團隊。**陌生人憑什麼相信他** | W10 | 4 | BE-G24 待銜接 | |
| | | 動作列。**只放後端今天做得到的動作**（私訊；成軍、結案由 `FE-J04` 接進插槽）—— 收藏、檢舉、應徵沒有後端，**不放灰掉的按鈕**；**進房不在詳情**（唯一入口是大廳的門，`room-entry-gate`，2026-09-18 改） | W5 | 3 | | Alarm｜之前這一列寫了還不存在的動作 |
| FE-B04 | 人才列表與詳情 | 讀取 profiles、Talent Card、Detail。**人才端不能只有讀取** —— 發案者的工作區在 `FE-M06` | W2 | 6 | | |
| FE-B05 | 搜尋與篩選 | 技能、每週工時、可開始時間、時區、預算範圍、角色類型。本地後端自己做 | W6 | 8 | BE-G05 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G05） |
| | | **真後端只有 20 筆 offset 翻頁**，銜接時這一項會退化。UI 要能誠實降級成「瀏覽」 | W6 | 3 | BE-G05 待銜接 | Alarm｜本地做得到、真後端做不到的典型 |
| FE-B06 | 匹配訊號 | **可解釋的，不是演算法分數**：技能符合 3/4、每週工時符合、時區重疊、預算符合、可開始日符合 | W7 | 6 | BE-G21 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G21） |
| FE-B07 | 收藏與儲存搜尋 | 收藏案件／人才、儲存搜尋條件 | W12 | 6 | BE-G23 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G23） |
| FE-B08 | 發現入口 | Marketplace 看板、Guild Hall 的熱門／缺人、分享的連結直接進 | W11 | 4 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| | | 從**公開招募活動**進入 —— 跟著 `FE-V02` 走 | W11 | 3 | | |
| | | 從**技能場景推薦**進入 —— 跟著 `FE-V12` 走 | W13–W16 | 3 | | Pending｜技能場景在 W13 之後 |
| FE-B10 | 新手導覽 | 第一次進來的人怎麼知道要幹嘛：發案 / 接案 / 找人 / 進 Office / 進 Project Room 各走一次 | W12 | 6 | | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G01、BE-G23） |
...
（最後那一步是 `FE-N` 那一組。**寫在圍籬外面** —— 圍籬裡的 ID 一律
被當成格式範例跳過，寫在裡面的話，`FE-N` 改名不會有任何東西發現。）

| ID | 項目 | 工作 | 週 | 點 | 阻塞 | 標記 |
|---|---|---|---|---|---|---|
| FE-M01 | Open Role | **角色是核心實體，要有自己的生命週期**：草稿／開放／暫停／已補滿／取消／重開 | W6 | 6 | BE-G10 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G10） |
...
| FE-M02 | 應徵 | **對一個 Open Role 應徵，不是對整個專案。** 報價、可投入時間、可開始日、簡短提案、作品連結 | W6 | 8 | BE-G10 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G10、BE-G20） |
| | | 送出前預覽；**同一個角色只能有一份有效申請**，重複送出要擋而且說清楚為什麼 | W6 | 3 | | |
| | | 報價：固定總價／時薪／志願／待議，含幣別。**只是欄位，不接金流** | W6 | 3 | BE-G20 待銜接 | |
| FE-M03 | 應徵的後續 | 送出後能不能改？撤回後能不能重投？發案者看過之後再改要不要留歷史？ | W7 | 5 | | Alarm＋Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排；沒定義的話兩邊會看到不一樣的內容 |
| | | 未被接受前可撤回 | W7 | 2 | | |
| FE-M04 | 我的應徵 | 列表：案子、角色、狀態、送出時間、最後更新。**這是接案者每天回來看的那一頁** | W7 | 5 | BE-G10 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G10） |
| | | 七種狀態：待處理／已查看／邀請洽談／已接受／已婉拒／已撤回／**已逾期**。逾期**由時間推導**，不靠發案者手動 | W7 | 5 | BE-G10 待銜接 | |
| | | 被婉拒時顯示原因（若有填）。**「已讀不回」是求職者最痛的事** —— 做不到強制回覆，至少讓逾期看得見 | W7 | 3 | | |
| FE-M05 | 應徵者管理 | 依角色分組、依匹配度排序。每筆顯示應徵者、匹配度、報價、可投入時間、提案摘要 | W7 | 6 | BE-G10 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G10） |
| | | 動作：看 profile、邀請洽談、婉拒（可填原因）、標記待議。**批次婉拒** —— 二十封不能一封一封回 | W7 | 5 | | |
| | | 開啟時自動標記「已查看」，讓對方看得到 | W7 | 2 | | |
| FE-M06 | 候選名單 | 發案者的工作區：短名單、標記、並排比較。**人才端目前只有讀取，這是缺的那一半** | W8 | 6 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-M07 | 主動邀請 | 從人才頁對**某個專案的某個 Open Role** 發出邀請，附邀請訊息與初始條件 | W8 | 8 | BE-G10 待銜接 | Alarm＋Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G10）；這條路徑原本整個不存在 |
| | | 狀態：待回覆／已接受／已婉拒／已撤回／**已過期** | W8 | 4 | BE-G10 待銜接 | |
| | | **裁決：接受邀請會產生一份 application，狀態直接是「邀請洽談」。** 邀請與應徵之後走**同一條管線** —— 兩套平行流程會讓 `FE-N`、`FE-J` 全部要做兩次 | W8 | 3 | | |
| FE-M08 | 我的邀請 | 收到的邀請列表、詳情、接受／婉拒。跟「我的應徵」放在同一個地方 | W8 | 5 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-M09 | 邀請的邊界 | 重複邀請同一人、對方已經應徵過同一角色、角色關閉或補滿後邀請失效、對方設定不接受陌生邀請（FE-A07） | W8 | 5 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-M10 | 媒合意圖 | 找案／找人／組隊／短期支援／只是交流。**`Looking For` 不該只是一段文字** —— 結構化、可搜尋、可暫停、**會過期** | W7 | 6 | BE-G14 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G14） |
...
| ID | 項目 | 工作 | 週 | 點 | 阻塞 | 標記 |
|---|---|---|---|---|---|---|
| FE-N01 | 洽談對話串 | 綁在**一份 application** 上的對話 —— 不是站內信，站內信找不到脈絡 | W9 | 8 | BE-G10 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G10） |
| | | application 有兩種來源：接案者自己應徵（`FE-M02`），或接受邀請（`FE-M07`）。**之後走同一條管線，thread 只有一種 owner** | W9 | 2 | | |
| FE-N02 | 條件修訂 | 報價、工時、開始日、角色範圍。**雙方都看得到修訂歷史** | W9 | 6 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-N03 | 約時間 | 洽談用的**一次性** Meeting URL（不自建視訊）。URL scheme 白名單與 `rel="noopener noreferrer"` | W9 | 3 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| | | **跟 `FE-J14` 的專案資源不是同一個東西**：這裡是一次性的，那裡是成軍後的持久資源。**成軍時不自動轉存** | W9 | 2 | | |
| FE-N04 | Offer | 角色、合作期間、每週工時、報酬形式、開始日 | W9 | 6 | BE-G10 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G10） |
| FE-N05 | Offer 的失敗路徑 | **有效期限**、發案者撤回、新版本是否讓舊版失效、同一角色能不能同時發給多人、最後一席被接受後其他 Offer 怎麼辦、接受當下角色已補滿的競態 | W9 | 8 | | Alarm＋Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排；這些不定義，成軍那一刻就會出事 |
| FE-N06 | 接受與雙方確認 | 接案者接受／婉拒／提出修改。**雙方確認之後才成為團隊成員** —— 被加入不等於本人同意 | W9 | 6 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-N07 | 房間權限 | 接受 Offer 後自動取得 Project Room 存取，不用再輸一次密碼 | W10 | 5 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| | | **退出或被移除時撤銷** —— 跟著 `FE-J08` 的退出與移除走 | W10 | 3 | | |
| FE-N08 | 房間門禁 | 進房：密碼 Modal、錯誤回饋、取得 room token、進出。**W4 就要有**，因為 W4 的 Project Room 場景需要它 | W4 | 6 | | |
| FE-N09 | 成軍時設房間密碼 | 成軍時**強制設定 Room Password** —— 跟著 `FE-N06` 的成軍走。原本是 `FE-N08` 的 W9 續行，2026-09-26 拆成獨立項目：`FE-N08` 的 W4 那段已經做了，整項搬到 W9 會改掉已發生的事實，而依賴只屬於這一段 | W9 | 3 | | Later｜demo 之後（2026-09-26）：跟著 `FE-N06` 的成軍走，`FE-N06` 排在 demo 之後 |
...
| ID | 項目 | 工作 | 週 | 點 | 阻塞 | 標記 | 負責人 |
|---|---|---|---|---|---|---|---|
| FE-J01 | 建立案件 | `POST /api/projects`（`ProjectCreate`：`title`、`body`、`needed_skills[]`、`seat_count`）；送出後立刻出現在看板與列表，7 天後自動過期（後端算 `expires_at`）。⚠️ **2026-09-17 縮成後端有的欄位**：Open Role、期程、預算、招募截止日全部沒有模型（`BE-G10`、`BE-G20`、`BE-G21`），表單上不放；`title`／`body`／`skills` 的上限前端自己訂（`FE-X05`） | W5 | 6 | | | |
| | | 草稿與發布分開。發布前預覽「別人會看到什麼」 | W6 | 3 | | | |
| FE-J02 | 編輯已發布的案件 | 改描述、改條件、新增／修改／關閉／重開 Open Role、延長或提前結束招募 | W7 | 8 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 | |
| | | 觸發入口：改了之後**套用 `FE-M01` 定義的規則**，**這裡不另外發明一套** | W7 | 4 | | | |
| FE-J03 | 我的案件 | 案件擁有者的管理入口：我發的每個案子、各自的狀態（招募中／已成軍／已結案）、直達成軍／結案。後端沒有 owner 篩選，前端用 `owner_id` 過濾已載入的頁面並誠實標明。⚠️ **2026-09-17 縮成後端有的欄位**：沒有應徵數、沒有「還缺哪些角色」（`BE-G10`） | W5 | 4 | | | |
| | | 複製舊案件重新發布 | W12 | 3 | | | |
| FE-J04 | 成軍與結案 | 後端只有 `recruiting → active → closed`。**成軍**：`POST /api/projects/{id}/form-team`（owner 設房間密碼）→ 狀態 active、大廳長出門（`GET /api/rooms`）、密碼可複製並一鍵私訊給隊員。**結案**：`POST /api/projects/{id}/close` → 門消失、座位整批清掉（`BE-G07`）。⚠️ **2026-09-17 改寫**：原本的「暫停／取消／已補滿」要靠 `BE-G22` 拆開的生命週期，demo 之前不做 | W5 | 5 | | | |
| | | 招募中／暫停／取消與「已補滿」的聚合推導（案件層只能暫停或取消，不能手動宣稱補滿） | — | — | BE-G22 待銜接 | Pending｜等後端拆開生命週期；兩層各有一個「已補滿」的話一定會不一致 | |
| FE-J05 | 執行狀態 | 準備中／進行中／待驗收／已完成／中止。**跟招募狀態是兩條獨立的線** | W10 | 5 | BE-G22 待銜接 | Alarm＋Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G22）；不拆的話算不出完成紀錄，信任訊號整層做不了 | |
| FE-J06 | 狀態轉移規則 | **誰能改、從哪個狀態能改到哪個、改動可不可逆。** 光列狀態名稱不夠 | W10 | 6 | | Alarm＋Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排；狀態很多但轉移規則不完整，是最容易出爭議的地方 | |
| FE-J07 | 團隊名單與角色 | 誰在隊上、擔任哪個角色、角色調整、同一人多角色、替補與重新開缺 | W10 | 8 | BE-G10 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G10） | |
| FE-J08 | 退出與移除 | 接案者退出、成員被移除（要有理由與紀錄）、退出後能不能看歷史、**owner 離開或移交** | W10 | 8 | BE-G22 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G22） | |
| FE-J09 | 結案確認 | **誰能把專案標成完成、另一方要不要確認、不同意的時候怎麼呈現。** 這直接決定 FE-T 的完成紀錄算不算數 | W11 | 6 | BE-G22 待銜接 | Alarm＋Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G22）；單方面宣稱完成的紀錄沒有信任價值 | |
| | | 中止的路徑：發案者取消、雙方同意中止、長期無活動。**只有 `closed` 的話全部被算成同一種結果** | W11 | 5 | | | |
| FE-J10 | 團隊脈搏 | Project Room 現在只有外部連結，那是書籤不是工作脈絡。要能回答：目前目標、最近發生什麼、下一個里程碑、誰在做什麼、需要什麼協助、新成員怎麼進入狀況 | W11 | 8 | BE-G12 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G12） | |
...
| | | 案件頁「私訊發案者」的入口、按對方分成對話串、在串裡回覆（同一個 `POST /api/messages`，`fe-k01-inbox` 已封存，另開 change） | W5 | 3 | | |
| | | 已讀／未讀與未讀數 | W7 | 4 | BE-G06 待銜接 | |
| FE-K02 | 交易必要通知 | 通知機制本身 ＋ 顯示位置（全域未讀標記 ＋ 我的應徵／我的邀請頁上的標記） | W7 | 6 | BE-G11 待銜接 | Alarm＋Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G11）；**沒有這個，W7 之後的媒合流程等於要使用者自己一直重整** |
| | | 應徵類事件：被查看、被婉拒、被邀請洽談 | W7 | 3 | | |
| | | 邀請類事件：收到邀請、邀請被接受或婉拒 | W8 | 2 | | |
| | | Offer 類事件：收到 Offer、Offer 被修改或撤回 | W9 | 2 | | |
| | | 成員異動：加入、退出、被移除 | W10 | 2 | | |
| FE-K03 | 回訪通知 | 新案件符合你的條件、收藏的案件有動靜、儲存的搜尋有新結果 | W12 | 5 | BE-G23 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G23） |
| | | 管道（站內／email／推播）之後決定。**先決定「訂閱什麼」** | W12 | 2 | | |
| FE-K04 | 場景 chat | Lobby / Room chat UI、訊息列表、輸入框；場景切換清理。**不落 DB，refresh 清空** | W3 | 5 | | |
| FE-K05 | 狀態文字 | 快捷狀態、自由輸入、**12 字硬限制**、清除；透過 WS `status` 同步，別人頭上看得到（渲染跟 `FE-W08` 的名字牌共用）。**不是 Looking For**（那是 `FE-M10`，要 `BE-G14`） | W6 | 5 | | |
...
| ID | 項目 | 工作 | 週 | 點 | 阻塞 | 標記 |
|---|---|---|---|---|---|---|
| FE-T01 | 事實型聲譽 | **不先做五星評價** —— 容易失真、互相做人情、懲罰新手。先做可驗證的事實：已加入／已完成的專案數、準時完成或中途退出、合作時間長度、對方確認過的貢獻、帳號年齡、回覆率與平均回覆時間 | W11 | 8 | BE-G24 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G24） |
| | | **要出現在做決定的那一刻**（案件詳情的發案者區塊、Talent Card），不是藏在個人頁裡 | W11 | 4 | | |
| FE-T02 | 作品與外部身分 | 作品集連結、GitHub／Figma／LinkedIn。**未驗證的一律標示「未驗證」** | W11 | 4 | | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G25） |
| | | 已驗證的外部帳號 —— 需要 OAuth，但**價值不是省一次密碼，是證明這個身分屬於我** | W13–W16 | 6 | BE-G25 `BE-拒` | Pending｜後端排除 OAuth，要做得先翻那個決策 |
| FE-T03 | 封鎖與靜音 | 封鎖、靜音、隱藏。**只要允許陌生人私訊、空間接近與即時聊天，這就是產品基本能力** | W12 | 6 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-T04 | 檢舉與處置 | 檢舉表單、處理狀態回報給檢舉人 | W12 | 5 | BE-G15 `BE-拒` | Alarm＋Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G15）；後端排除 `/api/admin/*`。**只有檢舉按鈕、沒有人能處置，等於沒有** —— 要嘛翻決策，要嘛不要做這個按鈕 |
| FE-T05 | 合作後回饋 | 雙向，且只在有明確合作結果之後開放（依賴 `FE-J09` 結案確認） | W12 | 5 | BE-G24 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G24） |
...
| FE-T07 | 隱私與內容政策 | 誰看得到 profile、資料保存多久、法律頁面 | W13–W16 | 5 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
...
| | | **名字牌**：把 `presence`／`snapshot` 的 `name` 畫在遠端玩家頭上，只畫附近／鏡頭內的，Status 由 `FE-K05` 接上同一塊牌子。可以不等 ProceduralAvatar 那 10 點 | W6 | 3 | | Alarm｜登入已做（`FE-A01`、`FE-A06`），名字送得到；**只剩沒畫**。雙瀏覽器互見的驗收在 `FE-R10`（別人負責） |
...
| FE-W17 | 視覺分區 | Marketplace 與 Office 的配置 | W4 | 11 | BE-G09 待銜接 | Alarm＋Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G09）；先裁決「視覺分區」還是「伺服器 scene」，否則做出來名不符實 |
...
| FE-W13 | 渲染預算 | 遠端角色的 instancing / LOD / 簡化。**40 人同畫面是架構決定，不是收尾優化**。量測與數字目標一起在這裡 | W5 | 10 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-W14 | VisualPolish | 像素／寶可夢風的視覺統一（使用者 2026-09-22 拍板、視覺主導、demo 前）：暖色像素調色盤（單一色票 `src/design/world.ts`）、依 color token 程序生成的像素貼圖（含木紋）、像素草地、低 dpr 最近鄰的像素化渲染、角色 Outline＋框臉髮型＋臉部細節與坐姿姿勢；固定 Camera 下的構圖與可讀性（角色要對地面有對比）；Avatar 組合檢查避免穿模。**取代原「Toy-like／圓角」方向** —— 像素是硬邊，圓角不再是目標。**與 `FE-X09` 同組「弱裝置、好看」**：低解析度渲染同時是省 GPU 的手段。像素化要放寬 world-canvas 的 DPR 判準（原限 1~2，像素模式放寬到 0.25）。漂亮坐在椅子上的框景與座位權威歸 `FE-J13`（本項只做坐姿姿勢）。**不做**：外部美術資產（sprite／tileset／GLB，那是 `FE-W15`） | W8 | 10 | | |
| FE-W18 | 牆面語意物件的可讀性 | **固定的 45° 俯視相機只看得見朝南或朝上的面** —— 貼在南北向牆上、面朝東西的東西在畫面上是一條細縫。量出來：`door-corridor` 螢幕橫向輪廓 0.22、`shelf-social` 0.40，而角色直徑是 0.5。做三件事：① 鋸齒狀西牆（每個槽位凹折出一個朝南的小牆面，門搬回牆上，全部維持 90° 整數倍）② 全域掃描判準（**必須區分結構牆與語意物件** —— 牆本來就該是細的）③ 修 `door-corridor` 與 `shelf-social` | W4 | 8 | | Alarm＋Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排；視角讓核心語意物件讀不出來，`CONTEXT.md`：分不出來的話 3D 就是一條很貴的導覽列 |
| FE-W20 | 資料驅動的世界看板摘要 | 兩塊看板的固定卡槽顯示**第一頁前幾筆的可辨識摘要**（案件標題／人才名字），**不假裝知道 total**；看板自己的載入／錯誤／空三種呈現（跟 `FE-X04` 共用語彙）；世界物件的資料訂閱生命週期；「看板上的摘要」與「按 E 開出來的面板」一致性的判準。目的是讓**沒有開面板的人**遠遠就看得出「這裡現在有東西／沒東西／暫時讀不到」 | W7 | 8 | | Alarm｜`FE-B01` 做完之後看板唯一的 affordance 是走過去按 E —— `CONTEXT.md`：那是一條很貴的導覽列。這一列是那份規格 `design.md` `D5` 明寫交代不了的缺口，兩個外部審查者都判定它不屬於那 8 點 |
| FE-W15 | 資產管線 | 紋理尺寸、壓縮、授權、快取、版本與 fallback | W5 | 4 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
...
| FE-R10 | Presence | Online snapshot / player status / offline cleanup / online count。「兩個瀏覽器互相看得見對方的名字」那條驗收 2026-09-26 拆成 `FE-R13`（它依賴 `FE-W08`，而項目層級的依賴不收只指一條驗收的） | W3 | 6 | | | 小玉 |
| FE-R13 | 兩個瀏覽器互見名字 | 兩個瀏覽器（不是兩個分頁 —— 分頁共用 cookie）各登入一個名字，**互相看得見對方的名字**。原本誤放在 `FE-A01` 的 tasks 4.2，轉交給 `FE-R10`，2026-09-26 從 `FE-R10` 拆出來成為有自己週次的項目。⚠️ **依賴 `FE-W08` 的名稱渲染能力** | W6 | — | | Done｜小玉 #608（2026-09-21，`6e23a2f`）：`tests/e2e/two-browsers-names.mjs` 用兩個隔離 cookie 的瀏覽器驗 `FE-R10` 的那條判準，規格在 `remote-players` | 小玉 |
| FE-R11 | RealtimeChat | Lobby / Room scene chat 送收；client memory 保留近期訊息，refresh 後清空 | W3 | 6 | | | |
| FE-R12 | 斷線與復原 | 重連後重新握手、重建 snapshot、**清掉舊的 remote players 避免鬼影**（snapshot 前先清 roster）；**單一重連迴圈**、指數退避＋ full jitter。⚠️ 後端每次重部署 WS 全斷，demo 期間也可能發生 | W6 | 5 | | | |
| | | **重連後自己的位置會回到 (0,0)**，狀態文字被清空 —— 要重送 | W6 | 3 | | | |
| | | 使用者看得見的連線狀態；**握手失敗只會被 close 1008，沒有 `err`**，UI 要能解釋 | W6 | 3 | | | |
| | | room token **8 小時 TTL** 過期後要重新 `/enter`（1008 時走 `/enter`，不是無限重連） | W6 | 2 | | | |
...
| FE-V02 | 公開活動物件 | 空間裡要有**正在發生、可旁觀、可加入**的事：某團隊在開 15 分鐘招募說明、某人開放 portfolio review、某桌在找一位設計師、某技能區有 mentor office hour、某專案在公開 demo、某工作桌標示「可打擾」 | W11 | 8 | BE-G27 待銜接 | Alarm＋Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G27）；**沒有這一項，3D 就只是很貴的導覽列** |
| | | 活動的建立與結束：誰在辦、在哪、什麼時候開始、預計多久、可容納幾人 | W11 | 5 | BE-G27 待銜接 | |
| FE-V03 | 可旁觀 | 站在外圍就知道「這裡在幹嘛」，不用先加入。旁觀者不計入參與者、不能發言（或只能提問） | W11 | 5 | BE-G27 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G27） |
| FE-V04 | 漸進式接近 | 看見 → 靠近（顯示更多）→ 旁聽 → 打招呼 → 正式申請。**每一步都要能退回上一步** | W11 | 5 | BE-G27 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G27） |
| FE-V05 | 群體訊號 | 不能只呈現「在線人數」。要能回答：人在哪裡 → 為什麼聚集 → 跟我有沒有關 → 我能不能旁觀 → 我怎麼低風險加入 | W11 | 5 | BE-G09、BE-G27 | Alarm＋Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G09、BE-G27）；**這條鏈不成立的話，Presence 只是裝飾** |
| FE-V06 | 分級可見度 | 「正在找 React 隊友」可以公開，「在某個保密專案工作」不行。呈現**經過同意、分級抽象**的活動。**規則由 `FE-A07` 擁有，這裡只負責在空間裡呈現** | W11 | 5 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-V07 | Office Presence | 看見附近成員的 Display Name / Status / Profile；工作桌 | W5 | 6 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-V08 | Room Presence | Project Room 顯示團隊成員、在線狀態、座位 | W11 | 5 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-V09 | 共享儀式 | 固定招募市集、demo day、office hours、配對時段、結案展示。**空間記憶** —— 知道某類人與機會通常出現在哪裡 | W12 | 6 | BE-G13 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G13） |
| FE-V10 | 冷啟動 | 第一個使用者進來，世界是空的。降低傷害：預先策展的公開案件、固定時段活動、展示非即時內容、**先集中一個技能社群或一種案件類型** | W12 | 5 | | Alarm＋Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排；這是市場營運問題，不是做一個功能就解決 |
| | | **假人只能用於教學，不能冒充真人。** 空世界不該用假在線使用者掩飾 | W12 | 2 | | |
| FE-V11 | 世界導覽 | 場景地圖、快速移動、各場景 online count | W13–W16 | 5 | BE-G09 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G09）；優先序低於媒合閉環 |
| FE-V12 | 技能場景 | Developer Office / Design Studio / AI Lab；Profile Skills 與推薦入口連動 | W13–W16 | 10 | BE-G09 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G09）；要有伺服器 scene 才做得出「同領域的人在這裡」 |
| FE-V13 | Personal Desk | Office 空位 Claim / Leave；工作桌顯示 Avatar、Skills、Status | W13–W16 | 8 | BE-G14 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G14）；優先序低於媒合閉環 |
| FE-V14 | Event Space | 活動列表、Detail、類型、Participants；Stage / Audience Area | W13–W16 | 12 | BE-G13 待銜接 | Later｜demo 之後（2026-09-19）：要靠後端沒有的 API（BE-G13）；地基是 FE-V02 的公開活動物件 |
...
| FE-X02 | 狀態管理 | TanStack Query Provider / Query Keys 規範；`ui` / `world` / `interaction` / `avatar` store（**高頻 position / animation 不寫入 store**） | W1–W2 | 6 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
...
| FE-X13 | 按鈕與表單的最小外觀 | ⚠️ **這一列是截圖抓到的，不是規劃出來的。** Tailwind 的 preflight 把 `<button>` **與 `<input>`** 的預設外觀清光了，所以在加上樣式之前，`/login` 與首次進入流程上的「複製鑰匙」「進入世界」在畫面上是**兩行漂著的字**，而**每一個輸入框都是隱形的** —— 18 條端到端斷言全綠，而那個 CTA 沒有人會認得出來。⚠️ **輸入框那一半原本沒有寫在這裡**，是使用者看到截圖後補的（原話：「登入畫面也太醜了 / 輸入匡也看不到」）；兩個外部審查者獨立確認它在這一列的範圍內，理由是**按鈕至少還有文字，空白的輸入框沒有任何內容**。**跟 `FE-W12`「門看不出來是門」是同一種形狀。** `FE-A06` 已經替它自己的流程補了最小可用的外觀；**`/login`（`FE-A01`，已封存）還沒有**。這一列做的是把那份最小外觀收成共用的東西並補上 `/login`，**不是** `FE-X05` 那種完整的表單一致性 | W2 | 3 | | Alarm｜**已封存的 `/login` 上，主要按鈕看不出來是按鈕** |
...
| FE-X07 | 無障礙 | DOM 面板的 aria、對比、鍵盤路徑。**3D 世界不承諾無障礙，但產品核心流程必須能純 DOM 完成** | W5 | 5 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
...
| FE-X09 | 相容矩陣 | WebGL2 偵測與友善提示、整合顯卡降級、**context lost**、`prefers-reduced-motion`、背景分頁節流、Safari / iOS | W8 | 5 | | |
| FE-X10 | 降級模式 | REST 活著但 WS 掛了／WS 活著但 DB 掛了／3D 掛了 —— 每一種的產品行為 | W5 | 4 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-X11 | 送出節流 | chat / status / 表單的前端節流。**後端沒有 rate limit**（BE-G16），這只擋得住守規矩的人 —— 要做，但不要當成防護 | W3 | 3 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-X12 | i18n | 文案抽出、語言切換、日期時間與時區格式化 | W13–W16 | 6 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-X14 | 行動版 DOM-only 模式 | `FE-X08` 決定的實作（原本是它的第二列；拆成自己的 ID 是因為決定已 `Done`、實作還沒開始，兩件事不該共用一個狀態）：`pointer: coarse` 判定、不進世界的 DOM 殼（看板／名片／收件匣／登入／深連結）、桌面小視窗的提示遮罩、「試試完整空間」／「回到精簡模式」兩個入口與記憶、`[Mobile]` 的線上狀態呈現（電腦版看得到但走不過去）。邊界照 `FE-X08` 那一列 | W13–W16 | 8 | | Later｜demo 之後（2026-09-19）：demo 範圍外的純前端功能，demo 後重排 |
| FE-X15 | 首屏載入順序 | 使用者打開 `/world` 到能動之間的順序：**DOM 殼先出來 → 身分 → 3D chunk → Canvas → WS**；Rapier（最大 chunk，711 KB）延後到 Canvas 之後才載；面板（看板／名片／收件匣）code-split、按 E 才載；載入中要有看得出在動的畫面，不是白屏。數字目標歸 `FE-O12`，這一列只管順序與拆分。⚠️ 2026-09-17 使用者：「不要讓使用者覺得卡或者讀取很慢」 | W7 | 5 | | Alarm｜今天是全部一起載：HTML 293 ms、canvas 1315 ms，中間什麼都看不到 |
| FE-X16 | DOM 介面的視覺與動線 | ⚠️ **這一列跟 `FE-X13` 同一個來源：使用者看了截圖。** 2026-09-18 原話：「非3D的網頁介面都醜醜的 / 沒有任何設計 / 大家看了會覺得不專業 / 不想使用 / 記得要優化 UI/UX / 也要規劃動線要使用者容易操作 / 不要造成混亂」。到 `FE-J04` 為止 DOM 那一半只有 `FE-X01` 的暫定 token（系統字體、預設藍、白盒子）與 `FE-X13` 的下限；每片都跑過 `ui-ux-pro-max` 但只查單點（對比、焦點），從沒有整體的視覺方向。截圖的證據：訪客提示、專案看板、聊天框三個浮層同時疊在畫面上。這一列做兩件事：（1）**視覺**：一套 DOM 的設計 token（字體、色票、間距、圓角、陰影；面板／卡片／按鈕／輸入框的層次）收進 `globals.css` 與 `src/design/`，`FE-X13` 的 controls 改成用它，`/login`、首次進入、看板、詳情、收件匣、聊天框全部套上；（2）**動線**：同一時間只開一個主浮層（開看板就收聊天框與訪客提示）、每個面板的標題／關閉／返回位置一致、每個畫面的主要動作唯一且最明顯。**不做**：3D 那一半（`FE-W09`／`FE-W14`）、新手導覽（`FE-B10`）、行動版（`FE-X14`）。驗收除了判準還要附**前後截圖** | W5 | 8 | | Alarm｜**使用者第二次說介面不專業、不想用；之後每一項都會長新畫面，先定風格才不用回頭重刷** |
| FE-X17 | 世界 HUD 臨場感（Gather Town） | ⚠️ **同一個來源：使用者看了 `/world` 截圖。** 2026-09-21 原話：「上方的 header 跟左邊的項目看起來都太簡陋、很廉價、跟畫面不符合、感覺 3D 跟網頁合在一起沒有整體感、沒有 3D 體驗、還是在操作網頁的感覺 / 人物的頭上應該要顯示名稱跟狀態」。`FE-X16` 做了 DOM 面板本身的視覺，但那是「把網頁做漂亮」；這一列做的是**讓 DOM chrome 跟 3D 世界融為一體**（Gather Town 式 HUD）。與 codex／gemini 討論後兩邊高度一致，做三件事：（1）**名牌含自己**：每個角色頭上有名字＋狀態（現在只有遠端有，`FE-W08` 明文「自己 SHALL NOT 有名字牌」要反轉；狀態沿用 `FE-K05` 的同一塊牌），深色半透明膠囊＋blur＋text-shadow、自己用 accent 區分；（2）**HUD 融合**：header 與左側面板從不透明白盒改成半透明毛玻璃（`backdrop-filter: blur`）、浮起（margin／圓角／柔和陰影）、收合優先（狀態縮成色點＋單行、聊天收成浮動鈕）；（3）**消滅原生表單感**：藍字邊框鈕改無邊框 hover 才浮、輸入框拔原生邊框、按鈕微動畫。**不做**：3D 場景本身（`FE-W09`）、行動版抽屜（`FE-X14`，只求不擋路）。驗收附**前後截圖**。反轉 `FE-W08`「自己沒名牌」需 spec MODIFIED delta | W7 | 5 | | Alarm｜**使用者第三次說介面像廉價網頁、沒有 3D 體驗；demo 第一眼** |
...
| 里程碑 | 目標週 | 驗收結果 | 靠哪些 |
|---|---|---|---|
| **M1** 世界技術成立 ＋ 資料層立起來 | W1 | 進得了 3D 世界、走得動、看得到別人；40 browser gate 過。同時把**唯一一份契約**與 **adapter 切換**做好 —— 之後所有功能都建在它上面 | FE-O01、FE-O02、FE-O09、FE-O10、FE-O11、FE-O14、FE-W01、FE-W02、FE-W03、FE-W04、FE-W05、FE-W06、FE-W07、FE-R01、FE-R02、FE-R03、FE-R04、FE-R05、FE-R06、FE-R07、FE-R08、FE-R09、FE-X01、FE-X02 |
| **M2** 自己的後端立起來，能跑 W1–W2 這一期的流程 | W2 | 本地 Route Handler 骨架 ＋ 可拋棄資料庫 ＋ 契約測試跑兩邊。加上登入、登出、Profile、Avatar、Inbox、共用清單容器。**後續的產品操作隨各能力追加**（Open Role 在 W6、invitation 在 W8⋯⋯），W2 不會有那些 | FE-O02、FE-O03、FE-O04、FE-O05、FE-O06、FE-O20、FE-O21、FE-A01、FE-A02、FE-A04、FE-A05、FE-A06、FE-A08、FE-B01、FE-B04、FE-B09、FE-K01、FE-T06、FE-X01、FE-X02、FE-X03、FE-X04、FE-X05、FE-X06、FE-X08、FE-X13 |
| **M3** Guild Hall 成形 | W3 | 統一風格的世界、Presence、場景 chat、狀態文字 | FE-W08、FE-W09、FE-W10、FE-W11、FE-W12、FE-W19、FE-R10、FE-R13、FE-R11、FE-K04、FE-K05、FE-X11 |
| **M4** 專案空間 | W4 | 場景切換、Project Room、Project Door、座位 | FE-V01、FE-W07、FE-W16、FE-W17、FE-W18、FE-N08、FE-J13 |
| **M5** 案件閉環接上真後端（2026-09-17 改） | W5 | `FE-O08` 演練 → 入口一致 → 建案 → 卡片 → 詳情與私訊 → 成軍／結案 → 我的案件 → 座位 → 對話串。**W6–W8 見〈demo 之前的核准順序〉**：狀態、名字牌、斷線復原、首屏載入、看板摘要、發表準備。⚠️ 底下 W6 起的里程碑是 2026-09-04 建在本地後端模型上的，**demo 之前不走** | FE-O08、FE-A06、FE-J01、FE-B02、FE-B03、FE-K01、FE-J04、FE-J03、FE-J13、FE-X03、FE-X16、FE-T06 |
| **M-demo** demo 可演 | W6–W8 | 照〈demo 之前的核准順序〉：狀態、名字牌、斷線復原、首屏載入、看板摘要、世界 HUD 臨場感、像素風視覺打磨，最後由發表準備把 demo 腳本那條閉環完整走一次。`M5` 的「W6–W8 見核准順序」指的就是這一列 | FE-K05、FE-W08、FE-R12、FE-X15、FE-W20、FE-X17、FE-W14、FE-O15 |
| **M6** 接案流程成立 | W6 | 順序是 `FE-J01` 建立案件 → `FE-M01` Open Role → `FE-B02/B03` 瀏覽與詳情 → `FE-M02` 應徵。**沒有案子就沒有東西可以應徵** —— 跨組的同一週要照這個順序 | FE-J01、FE-M01、FE-B02、FE-B03、FE-M02、FE-B05 |
| **M7** 兩邊都追得到進度 | W7 | 我的應徵（七種狀態）、應徵者管理（分組、批次婉拒、已查看）、案件編輯與管理首頁、招募狀態、匹配訊號、媒合意圖、**交易必要通知** | FE-M03、FE-M04、FE-M05、FE-J02、FE-B06、FE-M10、FE-K02 |
| **M8** 雙向媒合 | W8 | 候選名單、**主動邀請**、我的邀請、邀請的邊界 | FE-M06、FE-M07、FE-M08、FE-M09 |
| **M9** 從有意願走到成軍 | W9 | 洽談對話串、條件修訂、Offer 與失敗路徑、雙方確認、房間權限 | FE-N01、FE-N02、FE-N03、FE-N04、FE-N05、FE-N06、FE-N07、FE-N08、FE-N09 |
| **M10** 專案有生命週期 | W10 | 執行狀態、轉移規則、團隊名單與角色、退出與移除、相容對映。**同時開始部分銜接** | FE-J05、FE-J06、FE-J07、FE-J08、FE-J12、FE-N07、FE-O08、FE-O07、FE-O19 |
| **M11** 信任與空間的產品價值 | W11 | 事實型聲譽、作品與外部身分、結案確認、團隊脈搏；公開活動物件、可旁觀、漸進式接近、群體訊號、聯絡邊界 | FE-T01、FE-T02、FE-J09、FE-J10、FE-J14、FE-V02、FE-V03、FE-V04、FE-V05、FE-V06、FE-V08、FE-A07、FE-B08 |
| **M12** 有理由回來 | W12 | 收藏與儲存搜尋、回訪通知、封鎖與檢舉、合作後回饋、共享儀式、冷啟動、市場有效期 | FE-B07、FE-K03、FE-T03、FE-T04、FE-T05、FE-V09、FE-V10、FE-B10、FE-J11 |
| **M13** 遠期 | W13–W16 | 世界導覽、技能場景、Personal Desk、Event Space、行動裝置、i18n、可觀測性、外部帳號驗證 | FE-V11、FE-V12、FE-V13、FE-V14、FE-X14、FE-X12、FE-O16、FE-T02 |
...
前端項目彼此的先後。**`--check` 驗這張表**：被依賴項目的最晚週 ≤ 這一項的最早週（同週合法；`任一` 至少一個來得及）、不准有環、已完成的項目前置也要完成。
週次在這份 WBS 是**先後順序**，不是日曆週（`docs/DECISIONS.md` 2026-09-26），所以這裡的逆序是真的矛盾，修法是**改週次、不動依賴**。

文法：這一項恰好一個 ID；依賴只收 ID、用 `、` 分隔；關係 `全部`／`任一`；說明的 ID 不算依賴。
**只指某一條驗收的依賴不寫在這裡** —— 把那條驗收拆成有自己週次的項目（`FE-N09`、`FE-R13` 就是這樣拆出來的）。

| 這一項 | 依賴 | 關係 | 說明 |
|---|---|---|---|
| FE-M01 | FE-J01 | 全部 | 沒有案子就沒有角色可開（原本 `FE-M02` 那列 `FE-J01 → FE-M01` 鏈的前半） |
| FE-M02 | FE-M01 | 全部 | 應徵是對一個 Open Role，不是對整個專案 |
| FE-N01 | FE-M02、FE-M07 | 任一 | 應徵或邀請都會產生 application，洽談從它開始 |
| FE-N07 | FE-J08 | 全部 | 房間權限的撤銷跟著退出與移除走 |
| FE-N09 | FE-N06 | 全部 | 成軍時設密碼，成軍是雙方確認之後 |
| FE-B08 | FE-V02 | 全部 | 從公開活動進入，要先有活動物件 |
| FE-T01 | FE-J09 | 全部 | 沒有確認過的完成紀錄不算數 |
| FE-T05 | FE-J09 | 全部 | 合作後回饋接在結案確認之後 |
| FE-J05 | FE-J06 | 全部 | 執行狀態照轉移規則走 |
| FE-R13 | FE-W08 | 全部 | 協定送得到 `name`，要 `FE-W08` 把它畫出來這條驗收才跑得動 |
