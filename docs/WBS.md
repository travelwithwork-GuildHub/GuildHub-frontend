...
| BE-G20 | **沒有任何金額欄位** | 後端 `CLAUDE.md` 排除「點數、金流、置頂、付費曝光」，但**「預算欄位」跟「金流」是兩件事**，被混在一起排除了。預算揭露（媒合條件）／報價議價（協商）／收付款託管（金融）是三個層次，可以只做第一層 —— **一個接案平台不談錢很奇怪** —— **【沒答案就】**案件不顯示預算，並在 UI 明說「價格請私下談」，不要假裝這個維度不存在。 | 決策≤W5 | — | `待裁決` | Alarm｜要做就得先翻後端的產品決策 |
| BE-G21 | **專案沒有時間欄位** | `hours_per_week` 只描述人才供給，沒有描述專案需求。缺開始日、期程或截止日、每週投入需求、時區與同步時段、緊急程度、一次性或長期 —— **【沒答案就】**時間全部寫在 `body` 純文字裡，接受它搜尋不到也比對不了。 | 決策≤W5 | — | `BE-缺` | Pending｜等後端 |
| BE-G22 | **生命週期沒有拆開** | `recruiting → active → closed` 把「停止招募」和「專案完成」混成同一條線。要拆成招募狀態（招募中／暫停／已補滿／取消）× 執行狀態（準備中／進行中／待驗收／已完成／中止）。**不拆的話算不出完成紀錄，信任訊號整層做不了** —— **【沒答案就】**不做任何完成紀錄與聲譽功能，並明說平台不累積合作歷史。 | 決策≤W7 | — | `BE-缺` | Alarm｜它擋住整個 FE-T |
| BE-G23 | **沒有訂閱與收藏** | 收藏案件／人才、儲存搜尋條件、狀態變化推送都沒有資料可以掛 —— **【沒答案就】**不做回訪功能，接受使用者只在主動想起時才回來。 | 決策≤W10 | — | `BE-缺` | Pending｜等後端 |
| BE-G24 | **沒有合作紀錄與聲譽** | 已加入／已完成的專案數、準時或中途退出、合作時間長度、回覆率、帳號年齡 —— 一個都沒有。它依賴 BE-G22 先拆開生命週期 —— **【沒答案就】**不做任何信任訊號，並接受「每一次媒合都要從零開始相信陌生人」。 | 決策≤W8 | — | `BE-缺` | Alarm｜這是媒合平台的核心 |
...
| BE-G26 | **沒有聯絡邊界設定** | 誰可以私訊我、是否接受邀請、狀態對誰可見、離線是否接收通知 —— 全部沒有。**空間產品會放大接近感，邊界感要比普通網站更強** —— **【沒答案就】**所有人都能私訊所有人，並在 UI 明說這件事，讓使用者自己決定要不要填 profile。 | 決策≤W6 | — | `BE-缺` | Alarm｜陌生人可以無限私訊，使用者沒有任何自保手段 |
| BE-G27 | **沒有公開活動物件** | 「某團隊正在開招募說明」「某桌在找設計師」「某人開放 portfolio review」——空間裡沒有任何**正在發生、可旁觀、可加入**的東西。**沒有這個，3D 就只是一條很貴的導覽列** —— **【沒答案就】**接受 3D 只提供品牌與情緒價值，**並且不要再宣稱它是產品機制**。 | 決策≤W3 | — | `BE-缺` | Alarm｜它決定 3D 這條路成不成立 |
---
...
> **這一組是整份規劃原本最大的空白。**
>
> 現有規劃的「接案」只有一個動作：**寄站內信給發起人**。
> 那不是媒合流程，那是陌生人私訊。發案者收到二十封信，
> 無法比較、無法追蹤誰申請了哪個角色、也沒有任何結果狀態。
>
> **求職者最痛的就是「已讀不回」，而這份規劃連「已讀」都沒有。**

| ID | 項目 | 工作 | 週 | 點 | 阻塞 | 標記 |
|---|---|---|---|---|---|---|
| FE-M01 | 媒合意圖 | 進來的人此刻想做什麼：找案／找人／組隊做 side project／提供短期協助／只是交流。**`Looking For` 不該只是一段文字** —— 要結構化、可搜尋、可暫停，而且**會過期** | — | — | BE-G14 `BE-缺` | TBD｜要不要把意圖做成一級資料還沒裁決 |
| FE-M02 | 合作條件 | 預算（固定總價／時薪／志願／待議 ＋ 範圍 ＋ 幣別）、接案者報價、是否含後續維護 | — | — | BE-G20 `待裁決` | TBD｜**「預算欄位」跟「金流」是兩件事**，後端把它們一起排除了 |
| | | 時間：預計開始日、期程或截止日、每週投入需求、時區與同步時段、緊急程度、一次性交付或長期 | — | — | BE-G21 `BE-缺` | Pending｜後端只有人才端的 `hours_per_week`，沒有專案端的需求 |
| FE-M03 | 應徵 | 對**專案內的某一個 Open Role** 提出申請（不是對整個專案），附報價、可投入時間、簡短提案 | — | — | BE-G10 `BE-缺` | TBD｜整個 domain model 不存在 |
| | | 撤回申請 | — | — | BE-G10 `BE-缺` | TBD｜同上 |
| FE-M04 | 申請狀態 | 待處理／已查看／邀請洽談／已接受／已婉拒／已撤回／**已逾期**。逾期**由時間推導**，不靠發案者手動設定 | — | — | BE-G10 `BE-缺` | TBD｜依賴 FE-M03 |
| | | 「已讀」訊號要小心 —— 它會增加焦慮，也會讓人刻意不打開。**比已讀更該做的是承諾回覆期限**（「這位發案者通常三天內回覆」） | — | — | BE-G24 `BE-缺` | TBD｜需要回覆率的統計 |
| FE-M05 | 洽談 | 申請與成軍之間缺一整段：邀請文字／語音洽談、提出合作條件、修訂報價與時間、確認角色與責任。**「接受申請」到底是進入洽談還是正式入隊，要先分清楚** | — | — | BE-G10 `BE-缺` | TBD｜「接受申請」的語意還沒定 |
| FE-M06 | 雙方承諾 | offer／acceptance：角色、合作期間、工時、報酬或志願性質、加入日期、**雙方確認**。被加入不等於本人同意 —— 否則 Team Formation 只是發起人單方面管理名單 | — | — | BE-G10 `BE-缺` | TBD｜依賴 FE-M05 |
| FE-M07 | 生命週期拆開 | **「停止招募」和「專案完成」是兩件事。** 招募狀態（招募中／暫停／已補滿／取消）與執行狀態（準備中／進行中／待驗收／已完成／中止）要分開 | — | — | BE-G22 `BE-缺` | Alarm＋TBD｜不拆開就**算不出完成紀錄**，信任訊號整層做不了 |
| FE-M08 | 退出與失敗路徑 | 發案者取消、接案者退出、雙方同意中止、成員被移除、長期無活動、合作爭議。**只有 `closed` 的話，這些全部被算成同一種結果** | — | — | BE-G22 `BE-缺` | TBD｜依賴 FE-M07 的生命週期拆分 |
| FE-M09 | 匹配訊號 | **先不要做推薦演算法。** 資料量與案件流動性不足時，演算法只是把稀少內容重新排序。先做**可解釋的訊號**：技能符合 3/4、每週工時符合、時區重疊、預算符合、可開始日符合 | — | — | BE-G05、BE-G20、BE-G21 | TBD｜依賴合作條件先成為結構化欄位 |
| FE-M10 | 市場有效期 | Marketplace 很容易累積殭屍內容。案件到期、招募需求定期確認、人才可用狀態過期、長期未回覆降權、「目前仍在招募」重新確認。**對媒合產品來說，「資料還有效嗎」通常比搜尋功能多寡更重要** | — | — | BE-G19 `BE-拒` | Alarm＋TBD｜後端明文排除到期提醒與續期排程 |
| FE-M11 | 訂閱與狀態變化 | 收藏案件／人才、儲存搜尋條件、新案件符合條件、申請狀態更新、有人邀請或回覆、專案房有待辦。**先做「訂閱什麼」，通知用 email／推播／站內之後再決定** | — | — | BE-G23 `BE-缺` | TBD｜先決定訂閱什麼，通知管道之後再說 |
| FE-M12 | 團隊脈搏 | Project Room 現在只有外部連結，那是書籤不是工作脈絡。至少要能回答：這個團隊現在的目標、最近發生什麼、下一個里程碑、誰在做什麼、需要什麼協助、新成員怎麼快速進入狀況。**不做完整專案管理，但沒有脈搏的話 Office Presence 沒有可讀內容** | — | — | BE-G12 `BE-缺` | TBD｜要做到多深還沒裁決，不做完整專案管理 |
...
| ID | 項目 | 工作 | 週 | 點 | 阻塞 | 標記 |
|---|---|---|---|---|---|---|
| FE-T01 | 事實型聲譽 | **不要先做五星評價。** 它容易失真、互相做人情、懲罰新手。先建立可驗證的事實：作品集與外部帳號、已加入／已完成的專案數、準時完成或中途退出、合作時間長度、對方確認過的技能或貢獻、帳號年齡、回覆率與平均回覆時間 | — | — | BE-G24 `BE-缺` | TBD｜依賴 BE-G22 先拆開生命週期，否則算不出完成紀錄 |
| FE-T02 | 持續身分 | **匿名或化名本身不是問題** —— 很多創作者社群靠持續化名運作。問題是**可以被任意冒充**。要的是：唯一帳號與名稱歷史、帳號年齡、外部帳號驗證 | — | — | BE-G25 `待裁決` | TBD｜KYC 除非涉及託管付款或法規，否則不該優先 |
| FE-T03 | 聯絡邊界 | 誰可以私訊我、是否接受邀請、是否允許陌生人靠近／聊天、個人狀態對誰可見、哪些專案關係可以公開、離線時是否接收通知。**空間產品會放大接近感，所以邊界感要比普通網站更強** | — | — | BE-G26 `BE-缺` | TBD｜要做多細還沒裁決 |
| FE-T04 | 封鎖與檢舉 | 封鎖、靜音、檢舉、隱藏。**只要允許陌生人私訊、空間接近與即時聊天，這就是產品基本能力**，不是工程安全的附加項 | — | — | BE-G15 `BE-拒` | Alarm＋TBD｜後端排除 `/api/admin/*`。只有檢舉按鈕、沒有人能處置，等於沒有 |
| FE-T05 | 合作後回饋 | 雙向、且要在有明確合作結果之後才開放（依賴 FE-M07 的生命週期拆分） | — | — | BE-G24 `BE-缺` | TBD｜依賴 FE-M07 有明確的合作結果 |
...
原本的規劃裡，3D 提供的正好就是那條導覽列。這一組是缺掉的那一半。

| ID | 項目 | 工作 | 週 | 點 | 阻塞 | 標記 |
|---|---|---|---|---|---|---|
| FE-V01 | 公開活動物件 | 空間裡要有**可旁觀、可加入的正在發生的事**：某團隊正在開 15 分鐘招募說明、某人開放 portfolio review、某桌正在找一位設計師、某技能區有 mentor office hour、某專案正在公開 demo、某工作桌標示「可打擾／歡迎加入」 | — | — | BE-G27 `BE-缺` | Alarm＋TBD｜**沒有這一項，3D 沒有存在理由** |
| FE-V02 | 可旁觀 | 先站在外圍了解一場招募、討論或工作 session，再決定要不要加入 | — | — | BE-G27 `BE-缺` | TBD｜依賴 FE-V01 |
| FE-V03 | 漸進式接近 | 看見 → 靠近 → 旁聽 → 打招呼 → 正式申請。**不必一開始就寄陌生私訊** —— 這是空間相對於列表網站唯一真正的優勢 | — | — | BE-G27 `BE-缺` | TBD｜依賴 FE-V01 |
| FE-V04 | 群體訊號 | 不能只呈現「在線人數」。要呈現：人在哪裡 → 為什麼聚集 → 這件事跟我有沒有關 → 我能不能旁觀 → 我怎麼低風險加入。**這條鏈不成立的話，Presence 只是裝飾** | — | — | BE-G09、BE-G27 | Alarm＋TBD｜這條鏈是 3D 的產品價值本身 |
| FE-V05 | 分級可見度 | 「正在找 React 隊友」可以公開，「在某個保密專案工作」不行。要呈現的是**經過同意、分級抽象**的活動，還要有勿擾／只對隊友可見。**空間可讀性必須跟隱私控制一起設計** | — | — | BE-G26 `BE-缺` | TBD｜可見度分級要跟 FE-T03 一起設計 |
| FE-V06 | 共享儀式 | 固定招募市集、demo day、office hours、配對時段、結案展示。**空間記憶**：使用者知道某類人與機會通常出現在哪裡 | — | — | BE-G13 `BE-缺` | TBD｜依賴 BE-G13 的 Events 模型 |
| FE-V07 | 冷啟動 | 第一個使用者進來，世界是空的、Marketplace 是空的。降低傷害的作法：預先策展的公開案件、固定時段的招募活動、展示非即時內容（近期活動、過往成果）、**先集中一個技能社群或一種案件類型**，不要攤薄 | — | — | | TBD｜這是市場營運問題，不是只做一個功能就解決 |
| | | **假人只能用於教學，不能冒充真人。** 空世界不該用假在線使用者掩飾 | — | — | | TBD｜要不要放教學用假人還沒裁決 |
