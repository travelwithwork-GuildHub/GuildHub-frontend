...
| FE-X13 | 按鈕與表單的最小外觀 | ⚠️ **這一列是截圖抓到的，不是規劃出來的。** Tailwind 的 preflight 把 `<button>` 的預設外觀清光了，所以在加上樣式之前，`/login` 與首次進入流程上的「複製鑰匙」「進入世界」在畫面上是**兩行漂著的字** —— 18 條端到端斷言全綠，而那個 CTA 沒有人會認得出來。**跟 `FE-W12`「門看不出來是門」是同一種形狀。** `FE-A06` 已經替它自己的流程補了最小可用的外觀；**`/login`（`FE-A01`，已封存）還沒有**。這一列做的是把那份最小外觀收成共用的東西並補上 `/login`，**不是** `FE-X05` 那種完整的表單一致性 | W2 | 3 | | Alarm｜**已封存的 `/login` 上，主要按鈕看不出來是按鈕** |
