...
- **邊界狀態**: 已強制
- **證據**: tests/world-project-room-anchors.test.tsx:191、tests/world-project-room-anchors.test.tsx:87、tests/world-project-room-furniture.test.tsx:151、src/world/seats/SeatAnchorProjector.tsx:20、src/world/seats/SeatAnchors.tsx:21、src/world/layout/projectRoomLayout.ts:21
...
- **座標有限、畫面外 hidden**：同檔「一幀之後每個座標是有限數；畫面外 hidden…」（期望用 `framing.ts` 的 `isOnScreen` 算，不用投影器自己那條路）。
